 ...
 * | 층 | 잡는 것 | 못 잡는 것 |
 * |---|---|---|
 * | **이 파일**(`collect` 안에서 돈다) | **「돌지만 안 모은다」** — 데이터가 낡으면 exit 1 → 잡 실패 | **파이프라인이 아예 안 도는 것.** 안 돌면 이 파일도 안 돈다 |
 * | **`heartbeat.yml`**(따로 돈다) | **24시간 안에 `collect` 성공이 없다** — 잡 실패든 실행 부재든 → **메일** | 부분 장애 · 予告 실취득 · 자기 자신의 정지 |
 * → **이 파일이 「조용한 낡음」을 「잡 실패」로 바꾸고, 하트비트가 그 실패(또는 부재)를 「메일」로 바꾼다.**
 *
 * ⚠**옛 논거(「제품의 일부인 감시는 만료되지 않는다」)가 틀렸던 이유**: 이 파일은 파이프라인 **안**에서 돈다.
 * 파이프라인이 멈추면 **같이 멈춘다** — 멈춘 것을 멈춘 것이 알릴 수는 없다.
 * 화면의 신선도 띠도 같은 병이었다(빌드 시점 스냅숏이라 정지하면 영원히 초록이다 · `layout.ts:160`).
 *
 * ⚠**오프시즌 예외가 없다**(`staleDays` 기본 2) — 일본시리즈 뒤 `collect` 가 매일 실패하고
 * 하트비트가 6시간마다 메일을 보내게 된다. **11월 초 전에 정해야 한다**(`docs/operations.md`).
 *
