...
| **A7** | ⚠**2005~2007 分離ドラフト는 재시도로 안 풀린다 — A5 의 「재시도」 지시가 여기엔 안 통한다** | 경합 주석이 있는 구단이 **두 추첨 구획에 각각 1巡目**을 가지면 `firstRoundPick`(`store/src/draft.ts`)이 던지고 **그 구단의 4개 구획 전부**가 못 들어간다. 그런데 그건 分離ドラフト의 **정상적인 모양**이다(설계 §「한 해에 구획이 3~4개」). ⚠**뿌리는 `parseDraftBids` 가 `※` 로만 잘라 「어느 섹션의 주석인가」를 잃는 것**이라, **입력이 같으면 같은 예외가 난다 — 재시도로는 절대 안 풀린다.** 선언 범위 **22시즌 중 3시즌**이 걸린다 |
...
| **A11** | ⚠**A7 이 풀리기 전에는 2005~2007 이 영구 적색이다** | A7 로 일부 구단이 아예 안 들어오는데, 그 상태에서 **INV-N3 는 반드시 붉다**(선언 3 · 멤버 2). **A7 을 고치기 전에 그 3시즌에 불변식을 걸면 매번 붉고, 그 붉음은 결함이 아니다.** ⚠**안 쟀고 어느 문서에도 없던 상호작용이다** |
| **A12** | ⚠**`bids` 를 `null` 로 정하는 규칙이 코드에 없다 — 수집기의 첫 줄이 그것이어야 한다** | `null`(소스가 경합을 아예 안 쓴다) vs `[]`(말했고 이 구단은 안 겹쳤다)를 **`loadDraft` 가 기본값 없이 요구**한다. ⚠**파서는 이 판정을 못 한다** — 2019 히로시마도 `※` 0건이다. **시즌 전체를 봐야 갈린다.** 지금 그 규칙은 JSDoc 에만 있다 |
