...
| **A2** | ⚠**「개최 전」은 파서가 판정할 수 없다 — 호출자만 할 수 있다** | 파서는 **시계를 못 읽는다**(M6). 파서가 아는 것은 관측(`draftlist_*` 0건 · `開催要項` 있음)뿐이고, **과거 연도에서 같은 관측이 나오면 그건 마크업 붕괴다.** 그 연도가 과거인지 아는 것은 수집기다 |
