...
| **G2** | **시즌마다 경합 주석 건수를 세고, 0건인데 `bids: null` 판정을 안 했으면 멈춘다** | 이 파서는 **실패와 정답이 같은 모양**이다. M7 그물이 문법 붕괴는 잡지만 **`※` 자체가 사라진 개편**은 못 잡는다. ⚠**~~0건이면 멈춘다~~ 였고 그건 2026-09-05 에 거짓이 됐다** — 아래 |
| **G3** | **`DraftParseError` 가 한 건이라도 나면 그 시즌을 적재하지 않는다** | 부분 적재는 「그 해는 원래 그렇다」로 읽힌다 — 2018 오릭스 `bs` 사고와 같은 모양 |
| **G4** | ⚠**`確定`(당첨) 주석이 실린 실물을 픽스처로 고정한 뒤에 INV-N3 를 신뢰해라** | INV-N3 의 전제는 **당첨 팀도 상대를 선언한다**는 것인데, ⚠**저장소의 실물 픽스처에 `確定` 주석이 0건**이라 **그 전제를 저장소 안에서 재현할 수 없다.** 규칙 문서 §2 의 40문장 실측과 산술(2019: 5그룹 · 주석 14건 = 그룹 크기 합)이 뒷받침하지만 **그건 문서이지 시험이 아니다.** G1 과 같은 종류다 |

⚠**G1 을 「나중에」로 미루지 마라.** 2007 을 적재하는 커밋과 픽스처를 고정하는 커밋은 **같은 라운드**에 있어야 한다.

### ⚠G2 가 왜 바뀌었나 — 「0건」에 **세 번째 뜻**이 생겼다 (2026-09-05)

처음 쓸 때 「0건」은 두 가지였다: **⑴ 단독지명뿐인 해**(정답) · **⑵ `※` 가 사라진 개편**(붕괴).
N1 수정이 **⑶ 「그 시즌 소스가 경합을 아예 안 쓴다」**를 더했다 — **2023 12구단 전수 0/12** 가 실측이고,
그건 붕괴가 아니라 **진실**이다.

⚠**옛 문구를 그대로 뒀으면 2023·2024·2025·2026 이 영원히 멈춘다 — 경합뿐 아니라 지명 명단조차 못 들어간다.**
같은 문서의 **B2** 가 그 시즌들이 들어간 것을 전제하므로 **두 줄이 정면으로 모순이었다.**

⚠**이것이 이 계획에서 「고침이 문서를 낡게 한다」의 몇 번째인지 세지 마라 — 반복되는 것이 요점이다.**
**코드가 새 상태를 만들면 그 상태를 아는 문장을 전부 찾아 고쳐라.**
...
| **INV-1·2·3·5·6·7** | wikipedia 파서가 있어야 검사할 수 있다. ⚠**5가 빠져 있었다**(2026-09-05 정정 · 최종 검토) — 아래 참조 |

⚠**INV-5 는 이름이 충돌한다. 「이미 했다」고 읽지 마라.**

| 정본(`docs/sources/…-markup-rules.md` §5)이 말하는 INV-5 | 이 계획이 구현한 「INV-5」 |
|---|---|
| 머리행 **정확히 1개** · 데이터 행 폭 **정확히 13** · 배너 colspan **13** — **wikipedia 그리드 마크업** | 같은 `(season, kind, round_no, team)` 에 입찰 1건 — **npb 표 유일성**(구현자 스스로 「오늘 PK 가 이미 보장한다」고 인정) |

**둘은 다른 것을 잰다.** 이름이 같아서 이월 목록이 **5를 이미 처리한 것으로 세고 있었다.**
⚠**이 브랜치가 두 번 겪은 「다음이 받는다가 거짓이었다」와 같은 모양인데, 원인이
「배정 누락」이 아니라 「이름 충돌」이라 더 안 보인다.**
→ **정본의 INV-5 는 wikipedia 파서와 함께 온다.** 구현된 쪽은 그때 개명해라.

⚠**아래 두 줄은 이제 「이 계획 밖」이 아니다**(2026-09-06 정정 · 최종 검토 m2 · **내가 위 산문을
표 중간에 끼워 넣어 두 행이 표에서 떨어졌고, 그 사이에 둘 다 구현됐다**):

| 옛 「후속」 | 지금 |
|---|---|
| ~~**수집기**(archiver) · 연 1회 배치라 층이 다르다~~ | ✅ **구현됨** — `packages/archiver/src/{draft,cli-draft}.ts` · 계획 `2026-09-05-draft-collector.md` · 런북 `docs/operations/draft-backfill.md` |
| ~~**화면** · 데이터가 들어간 뒤~~ | ✅ **구현됨** — `packages/web/src/draft-page.ts` · 계획 `2026-09-05-draft-screen.md` · **배포물 9장** |
...
| **A6** | ⚠**불변식 검사는 「그 시즌의 전 구단이 들어온 뒤」에만 걸어라** | `scripts/test/draft-invariants.test.ts` 의 불변식(현재 6종)은 **한 시즌이 완결된 상태**를 전제한다. 적재는 **구단 단위**이므로, 중간 상태에서 **INV-N1 이 붉은 것은 결함이 아니라 시즌이 덜 들어온 것**이다. ⚠**검사를 거는 자리는 그 시즌 마지막 구단 직후다.** 중간에 걸면 **매번 헛불이 나고, 헛불이 일상이 되면 진짜 위반도 안 읽힌다** |
| **A7** | ~~**2005~2007 分離ドラフト는 재시도로 안 풀린다**~~ → ⚠**해결됐다**(2026-09-06 정정 · 최종 검토 m3). **파서가 `DraftBidRow.kind` 를 싣고 `firstRoundPick` 이 구획을 먼저 물으면서 그 예외가 안 난다.** 실측: **2005·2006·2007 이 전부 적재되고** 런북이 「18시즌 6종 전부」라고 적는다. ⚠**원인도 A7 이 아니었다** — 어휘(`競合`·`獲得`)와 **`（第N回）` 가 페이지 순서가 아니라 구획 안 순서**라는 구조였다. 아래는 **그때의 서술**이다 | 경합 주석이 있는 구단이 **두 추첨 구획에 각각 1巡目**을 가지면 `firstRoundPick`(`store/src/draft.ts`)이 던지고 **그 구단의 4개 구획 전부**가 못 들어간다. 그런데 그건 分離ドラフト의 **정상적인 모양**이다(설계 §「한 해에 구획이 3~4개」). ⚠**뿌리는 `parseDraftBids` 가 `※` 로만 잘라 「어느 섹션의 주석인가」를 잃는 것**이라, **입력이 같으면 같은 예외가 난다 — 재시도로는 절대 안 풀린다.** 선언 범위 **22시즌 중 3시즌**이 걸린다 |
| **A8** | ⚠**검사기를 모듈로 꺼낼 때 `assertClean` 도 같이 꺼내라** | 검사기 자체는 `{checked: 0, violations: []}` 를 낸다 — **분모 0을 실패로 만드는 것은 시험 하네스의 `assertClean` 뿐**이다. 그것을 두고 검사기만 꺼내면 **「검사 대상 0건」이 「위반 0건」으로 조용히 통과**한다 |
| **A9** | ⚠**문서층 그물의 오탐 방향을 재라 — 아직 아무 데도 안 적혀 있다** | 문서층은 푸터 위 **전문**을 세는데 `chunks` 는 **첫 `※` 앞 텍스트를 버린다.** 사이드바·헤드라인에 「겹침+추첨」이 30자 안에 붙으면 **정상 페이지가 통째로 실패**한다. **픽스처 10장 중 0건**이지만 ⚠**264페이지(22시즌 × 12구단)는 안 쟀다** |
| **A10** | ⚠**INV-2 는 이월이 아니다 — 지금 npb 데이터만으로 계산된다** | 규칙 문서 INV-2(「1巡目k 낙첨 구단 집합 = 1巡目k+1 참가 구단 집합」)는 `draft_bid(round_no, team, won)` 이면 충분하고 **wikipedia 도 `rivals` 도 필요 없다.** 이월 사유(「wikipedia 파서가 있어야」)가 **틀렸다.** ⚠그리고 그것이 정확히 **「한 구단이 1회차 낙첨 없이 2회차에 나타나는」 모양**을 잡는 검사다 |
| **A11** | ~~**A7 이 풀리기 전에는 2005~2007 이 영구 적색이다**~~ → ⚠**A7 이 풀려서 이 조건도 사라졌다**(2026-09-06 정정). 실측으로 **그 3시즌이 6종 전부 검사 가능**하다. 아래는 그때의 서술이다 | A7 로 일부 구단이 아예 안 들어오는데, 그 상태에서 **INV-N3 는 반드시 붉다**(선언 3 · 멤버 2). **A7 을 고치기 전에 그 3시즌에 불변식을 걸면 매번 붉고, 그 붉음은 결함이 아니다.** ⚠**안 쟀고 어느 문서에도 없던 상호작용이다** |
| **A12** | ~~**`bids` 를 `null` 로 정하는 규칙이 코드에 없다**~~ → ⚠**낡았다**(2026-09-06 · `decideBids` 가 구조적으로 강제한다 · 남은 것은 A14 가 소유한다)。예전 서술 — 수집기의 첫 줄이 그것이어야 한다** | `null`(소스가 경합을 아예 안 쓴다) vs `[]`(말했고 이 구단은 안 겹쳤다)를 **`loadDraft` 가 기본값 없이 요구**한다. ⚠**파서는 이 판정을 못 한다** — 2019 히로시마도 `※` 0건이다. **시즌 전체를 봐야 갈린다.** 지금 그 규칙은 JSDoc 에만 있다 |
| **A13** | ⚠**`[]` 로 거짓말하는 수집기는 아무 데도 안 걸린다** | 타입은 `null` 을 **가능하게** 할 뿐 **강제하지 못한다.** 시즌 전체가 유도된 단독으로 채워져도 **검사 6종이 전부 초록**이다. ⚠**「전원 단독인 시즌」 검사(INV-N4)를 일부러 안 넣었다** — 근거가 될 실측이 **2013 10/12 · 2019 10/12 두 시즌뿐**이라 임계값을 정할 수 없고, **근거 없는 임계값은 헛불이 되고 헛불이 일상이 되면 진짜 위반도 안 읽힌다**(A6 과 같은 이유). ⚠**그런데 사유가 그것만이 아니다** — **임계값 없는 버전이 실재하고**(「`bids !== null` 인 구단이 있는데 경합 그룹이 0개면 실패」) 그 근거는 두 시즌이 아니라 **주석이 존재한 7시즌 표본 전부에서 경합 ≥1** 이다. **그런데도 안 넣는 진짜 이유는 「이미 다른 층에 있다」**다 — **`G2` 가 수집기 층에서 정확히 그것을 본다.** 층을 둘로 늘리면 M1 이 싫어하는 모양이 된다 |
| **A14** | ⚠**2023~2026 은 불변식 6종 중 5종이 구조적으로 붉다 — A11 의 2005~2007 과 같은 것인데 이쪽이 더 크다** | `bids: null` 이면 그 시즌 `draft_bid` 가 **0행**이라 **INV-4 는 구조적으로 만족 불가**(분모는 `draft_pick` 에서 나와 슬롯이 살아 있고 획득은 영원히 0 · 실측 4구단에서 **4/4 위반**) · INV-4′·INV-5·INV-N1·INV-N3 는 **분모 0** 이라 `assertClean` 이 실패시킨다. ⚠**INV-4 의 메시지 「확정된 획득이 0건이다」가 파싱 버그처럼 읽히는데 실제 뜻은 「이 시즌 소스가 경합을 안 쓴다」**다. ⚠**뿌리**: `null`/`[]` 구분이 **입력 층에만** 있고 **DB 에 안 남는다** — `draft_event` 가 `(season, kind)` 단위라 정확히 그 자리인데 비어 있다. **남길지 결정해라. 안 남기면 검사기는 영원히 그 시즌을 「획득이 사라졌다」로 읽는다** |
| **A15** | ⚠**주석 짝 검사의 실패 표면이 넓어졌다 — A9 와 같은 방향이다** | `stripComments` 가 **문서 전체**에 걸리므로 닫히지 않은 `<!--` 가 **푸터 아래·`<script>` 문자열 안**에 있어도 지명·경합 파서가 던진다(옛 판은 푸터 아래를 아예 안 봤다). HTML5 축약 주석 `<!-->`(브라우저는 빈 주석)도 던진다. **픽스처 10장 짝 108/108 이라 오늘 0건**이지만 ⚠**264페이지는 안 쟀다** |
| **A16** | ⚠**「한 `(season, kind, team, round_no)` 에는 출처가 하나뿐」이 어디에도 안 적혀 있다** | `origin` 을 PK 에서 빼고 **DELETE 만 가리는** 구조는 그 규칙 위에서만 안전하다. 실측: 같은 키에 두 출처를 넣으면 **PK 충돌로 던지고 롤백**된다(남의 행은 무손실). ⚠**정말로 공존이 필요해지면 PK 재구축이지만, 4표는 아카이브에서 완전히 재유도 가능하고 규모가 작다(22시즌 × 12구단) — 되돌릴 수 있는 결정이다** |
| **A17** | ⚠**파서 네 진입점의 「경계 배선」이 주석으로만 지켜진다 — 이 브랜치에서 그 주석이 실제로 되돌림을 유도했다** | `parseDraftPicks`·`parseDraftBids` 는 `visibleScope`(주석 제거 + 푸터 컷), `parseDraftYears`·`parseDraftTeamSlugs` 는 `stripComments`(푸터 컷 없음)를 쓴다. **표는 주석이라 배선이 바뀌면 또 낡는다.** ⚠실제로 한 번 낡아서 「네 파서가 같은 경계를 쓴다」가 됐고, **그건 구현자가 자기 재독에서 방금 잡은 결함을 다시 유도하는 문장**이었다. → **`scripts/test/doc-figures.test.ts` 가 소스와 문장을 대조하는 방식**(§2-2-1 에서 이미 한다)을 여기에도 붙일 수 있다. **이번엔 안 했다 — 안 한 것이지 못 한 것이 아니다** |

⚠**A7 은 실물로 확인되지 않았다.** 그 연대의 커밋된 픽스처는 **2006 요미우리 1장**뿐이고,
하필 希望入団枠 을 써서 大学生・社会人 이 3巡目부터라 **안전한 케이스**다 —
**12구단 중 1구단 · 3시즌 중 1시즌.** 코드와 시험이 이 조건에서 던지는 것은 확정이지만,
**실제로 몇 구단이 걸리는지는 안 쟀다.**
...
| **B2** | ⚠**2023~2026 은 `draft_bid` 가 통째로 0행이다 — 「데이터 없음」이 아니라 「소스가 안 쓴다」다** | npb.jp 가 2023 부터 경합 주석을 안 쓴다(**2023 12구단 전수 0/12** 실측). 그래서 그 시즌은 `bids: null` 로 넣고 **단독지명 유도도 안 돈다.** ⚠**화면이 그것을 「데이터 없음」으로 그리면 거짓**이다 — 경합은 실제로 있었고 우리가 그 소스를 안 가진 것뿐이다. **M12 4상태**(로딩 / 데이터없음 / 수집실패 / 시즌외)로도 안 갈리는 다섯 번째 상태이고, 경합은 **wikipedia 파서(후속 계획)가 온 뒤에 채워진다** |
