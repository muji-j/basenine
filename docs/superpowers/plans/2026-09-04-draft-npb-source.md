...
| **A13** | ⚠**`[]` 로 거짓말하는 수집기는 아무 데도 안 걸린다** | 타입은 `null` 을 **가능하게** 할 뿐 **강제하지 못한다.** 시즌 전체가 유도된 단독으로 채워져도 **검사 6종이 전부 초록**이다. ⚠**「전원 단독인 시즌」 검사(INV-N4)를 일부러 안 넣었다** — 근거가 될 실측이 **2013 10/12 · 2019 10/12 두 시즌뿐**이라 임계값을 정할 수 없고, **근거 없는 임계값은 헛불이 되고 헛불이 일상이 되면 진짜 위반도 안 읽힌다**(A6 과 같은 이유). **실측이 늘면 그때 넣어라** |
