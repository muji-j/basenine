...
### ⚠그리고 「두 번」도 「세 번」도 틀린 수였다 — 훑고 나서야 분모가 나왔다

**추적 파일 463개 전수**를 훑었다(2026-09-06 · `git ls-files -z` → 바이트로 `\x00` 세기).

| | 값 |
|---|---|
| NUL 이 든 추적 파일 | **20** |
| 그중 정상(gzip 픽스처 `.html.gz`) | **20** |
| **소스·문서** | **0** ← 아래 둘을 고친 뒤 |
...
| 파일 | 무엇이었나 | 동작이 바뀌나 |
|---|---|---|
| `scripts/test/draft-invariants.test.ts:419` | 맵 키 구분자 `` `${r.season}<NUL>${r.group_key}` `` | **안 바뀐다** — `\u0000` 이스케이프로 바꿨고 런타임 값이 같다(15본 중 15본 통과) |
| `scripts/archive-guard.ts:138` | `.pop() ?? "<NUL>"` — `""` 를 쓸 자리 | **안 바뀐다** — `split` 이 항상 1개 이상을 내므로 `??` 자체가 안 탄다(8본 중 8본 통과) |

⚠**두 번째는 이 브랜치가 아니다** — `2026-08-24` 이전부터 있었다(`git log --follow`).
즉 이건 **이 브랜치의 버릇이 아니라 저장소에 반복되는 모양**이고,
「이번에 두 번 났다」로 적으면 **다음 사람이 자기 브랜치만 의심한다.**

⚠**세지 말고 훑어라.** 횟수는 기억에서 나오고 기억은 틀린다 — **분모는 스윕에서만 나온다.**
`grep` 은 NUL 이 든 파일을 「Binary file … matches」로 지나가므로 **`grep` 으로는 못 찾는다.**
