...
> ⚠**이 태스크는 이미 구현됐다**(`e5fd2c8`). **아래 코드는 초판이고 세 곳이 틀렸다** —
...
**같은 222요청을 상대에게 다시** 물린다(L7: 안 보내는 요청이 가장 안전한 요청).
