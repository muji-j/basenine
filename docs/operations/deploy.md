...
| 3 | Pages 무료 한도 대비 산출물 크기 | **9,569파일 / 391MB**(4시즌 · 2026-08-17 실측). Pages는 **배포당 파일 20,000개**·파일당 25MiB. ⚠**시즌당 약 2,400파일씩 는다** — 지금 48%이고 **8시즌(2019년경)에서 상한에 닿는다.** 더 소급하려면 배포를 시즌별로 나누거나 페이지 생성 방식을 바꿔야 한다. 시즌을 추가할 때마다 이 칸을 다시 재라 |
| 4 | 모바일 실기 확인 | 미확인 |
| 5 | wRC+ 외부 공표값 대조 | 미실시 |
