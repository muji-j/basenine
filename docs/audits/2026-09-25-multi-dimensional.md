...
> **2026-09-26 후속 — 정본 대조 완료.** 전제 「13勝以上」은 **1차 출처로 확정**됐다(npb.jp 표창 페이지의 【※】 주석 ·
> 2020パ 는 「10勝以上」). 승수 기준 1위 = 수상자 **16/16** · 규정투구회 기준 **12/16** — 어긋나는 4곳이 NPB 가
> 주석을 붙인 4곳과 같다. 위 「2020パ 는 특례가 있었을 수 있어」도 풀렸다(10勝 규정 · 1차 확인).
> 대조가 새로 드러낸 것: 규정투구회는 NPB 통계표(勝率リーダーズ)의 기준으로 보인다(要確認). 그래서 **순위 기준은 두고
> 거짓 문구를 고쳤다**(수정안 ①의 「자격을 승수로」도 ②의 「이름을 뺀다」도 아닌 제3안 — 순위와 표창이 다르다는 것을 화면이 말한다).
> 근거·인용·수상자 표는 `docs/sources/2026-09-26-winpct-title-qualification.md`.
...
## 10. 2026-09-26 후속 — 처리 현황과 미검증 16건 반증

### 10-1. 고친 것 (`muji-j/basenine` · 전부 머지 커밋)

| PR | 항목 | 검토 |
|---|---|---|
| #18 | C5 · C6 | 3중(1차 review-specialist · 2차 deep-reviewer · 3차 gpt-6-astra xhigh) |
| #19 | C1 · C2 (+ 미검증이던 C2 는 여기서 시험으로 확정) | 3중 + 반영분 재검토 |
| #20 | C12 — 전제(13勝以上)를 1차 출처로 확정한 뒤, 순위 기준은 두고 거짓 문구를 고쳤다(`docs/sources/2026-09-26-winpct-title-qualification.md`) | 교차 모델(gpt-5.6-sol high) |
| #21 | W1 · W4 · W9 · C4 | 교차 모델 + 반영분 재검토 |

### 10-2. 미검증 16건 블라인드 반증 (C2 는 #19 로 해소돼 제외)

반증자는 **다른 벤더 모델**(`shiro-core:gpt-reviewer` · `-m gpt-5.6-sol` · `high` · 판정 스키마 강제)이고 main `208917f` 을 읽었다.
**등급은 반증자가 현재 영향으로 다시 매긴 것**이다(감사 때 등급을 옮기지 않았다).

| ID | 판정 | 등급 | 확신도 | 한 줄 |
|---|---|---|---|---|
| C7 | 결함 실재 | P2 | 확정(재현) | `profile_fetched_at` 에 사이드카 취득 시각이 아니라 **적재 실행 시각**이 들어간다(M4) |
| C8 | 결함 실재 | P2 | 확정(재현) | 통산 표 id 자체가 사라지면 `parseCareer` 가 빈 배열을 내고 **기존 통산 행이 조용히 삭제**된다(M7·M11) |
| C10 | 결함 실재 | P2 | 확정(재현) | 부분 수집일이 `MAX(played)` 가 된 뒤 다음 실행이 실패하면, 따라잡기 창이 그다음 날부터라 **진행 중이던 경기를 다시 안 받는다** |
| W2 | 결함 실재 | P2 | 확정(실측) | 스파크라인 선·눌린 ★ 가 팀색 그대로라 12구단 중 11구단이 어느 한 테마에서 3:1 아래 |
| W6 | 결함 실재 | P2 | 확정 | 헤더 검색이 구단명으로 찾아 「すべて見る」로 보내는데 선수 목록 `?q=` 는 구단명을 몰라 **0명** |
| W7 | 결함 실재 | P2 | 확정 | `?vs=` 가 今季 표만 좁혀, 通算에 있는 대전을 두고 빈 상태와 원시 ID 를 보인다 |
| W8 | 결함 실재 | P2 | 확정 | 공유 URL 한쪽이 색인에 없으면 **유효한 쪽까지** 조용히 버리고 「未選択」 (C4 수정 뒤에도 남는다) |
| C3 | 결함 실재 | P3 | 확정 | 목록을 닫아도 늦게 온 색인 응답이 다시 연다 |
| C9 | 결함 실재 | P3 | 확정(변이 재현 · 보유 아카이브 실발생 0건) | 선수 링크를 못 읽은 박스 행이 격리 기록 없이 사라진다 |
| C11 | 결함 실재 | P3 | 확정 | `accounted > pa` 가드는 구조상 발화할 수 없다(23종 결과 0/23) — 주석이 효력을 과장한다 |
| C13 | 결함 실재 | P3 | **코드상 성립 · 재현 미완** | 「ゴロアウト率」 분모가 실책 출루·野選을 아웃으로 센다 — 반증자가 실행기 장애로 값 비교를 못 했다 |
| W10 | 결함 실재 | P3 | 확정(에뮬레이션) | 모션 감소 설정의 전역 `transition-duration` 이 `transition-property: all`(초기값)과 만나 사실상 transition:all |
| C14 | 결함 실재 | P3 | 확정(**루트 직접 확인** · 다른 벤더 반증 아님) | 打順一巡 각주에 「その日**good**投球」 — 영어 낱말이 배포된다 |
| W11 | 결함 실재 | P3 | 확정(**루트 직접 확인** · 다른 벤더 반증 아님) | `pages.ts` 주석의 「822KB · DOM 25,851요소」 — 지금 `ranking.html` 1,447,912 B |
| **W3** | **반증됨** | — | 확정 | 「초점 전후 계산값이 같다」가 틀렸다 — `:focus-visible` 결합 선택자가 라벨에 stroke 를 붙인다 |
| **W5** | **갈림** | — | 해석 문제 | WCAG 2.5.8 **인라인 예외** 적용 여부 — 찾은 쪽(「flex 아이템이라 인라인 문장이 아니다」)과 반증자(「span 안 라벨과 나란한 인라인 텍스트」)가 반대다. 결함으로 올리지 않고, 표적 간격을 넓히는 쪽은 선택 사항으로 둔다 |

**분모**: 16건 중 결함 실재 14 · 반증됨 1 · 갈림 1. 확신도 — 재현 확정 11 · 코드상 성립(재현 미완) 1(C13) · 루트 직접 확인 2(C14 · W11).

### 10-3. 이 라운드의 운영 관찰

- ⚠**첫 반증 워크플로가 통째로 죽었다** — 4 에이전트 모두 약 23분 뒤 `API Error: Can't reach the API server (ENOTFOUND)`. 같은 시각 codex 도 `Reconnecting... waiting for network` 로 멈췄다(07:27~07:28 UTC). **결과는 「0건」이 아니라 「안 쟀음」**이었다.
  두 묶음(client · window-metrics)은 codex 로그 끝에 최종 판정 JSON 이 남아 있어 회수했고(`ok` 는 client true · window-metrics false), 나머지 두 묶음은 네트워크 복구 뒤 다시 돌렸다.
- window-metrics 묶음은 반증자 쪽 Windows 실행기가 `0xC0000142` 로 죽어 C14 · W11 을 못 쟀다 — 그 둘은 grep·파일 크기만으로 판정되는 사실이라 루트가 직접 쟀다(위 표에 표기).

### 10-4. 처리와 새로 드러난 것 (2026-09-26 · 계속 갱신)

| 항목 | 상태 |
|---|---|
| C7 · C8 · C9 · C11 | **PR #22 머지**(3중 검토 — 3차 `gpt-6-astra` 의 P2 「신규 선수 + 투구 표 id 동시 변경 → 조용히 빈 통산」을 막은 뒤 반영분 집중 재검토) |
| C10 | **PR #23 머지** — 설계 콜드 리뷰 → 3중 검토에서 「since 하나만」이 부분 실패 경로로 무너져(3차 P2 · 2차가 같은 경로를 독립 발견) 최근 7일 소급으로 바꿈 → 반영분 집중 재검토 0건 |
| C3 · W6 · W7 · W8 | 가지 `fix-client-c3-w6-w7-w8` — 교차 모델 검토 P2(복원 대기 중 로딩 상태 없음 · M12)까지 반영 |
| W2 · W10 · C13 · C14 · W11 | 남음 |

**PR #22 검토에서 새로 드러난 것 — 결함 목록에 더한다(아직 반증 전 · 고치기 전에 반증부터):**

| 새 ID | 등급(찾은 쪽) | 내용 | 조용한가 |
|---|---|---|---|
| **N1** | P3 | 링크를 못 읽은 투수가 선수 표에 없을 때 경과 적재의 `pa_event.pitcher_id` 외래키 오류가 처리 안 된 예외로 올라와 **그 경기뿐 아니라 뒤따르는 전 경기·명단 보충까지 멈춘다**(`packages/store/tools/load-archive.ts` 의 `writeGameGuarded` 호출이 try 밖) — 이 PR 전부터 있던 경로 · 보유 0건 | 아니다(종료 1) — 다만 원인이 남으면 매일 같은 자리에서 멈춘다 |
| **N2** | P3 | `（投手交代） 旧 → 新` 에서 **新의 링크만** 없으면 `ids.at(-1)` 이 旧가 되어 이후 타석이 **이전 투수에게 조용히 귀속**된다(`packages/parser/src/playbyplay.ts` · 변이 실측 83타석 중 5) — 보유 경과 7,805장의 선발 행 15,066 · 교대 행 48,291 전부 링크가 있어 발생 0건 | **그렇다** |
| **N3** | P3 | 선수 프로필에 「옛 판이 새 판을 덮지 못하게」 하는 가드가 없다(경기 표에는 있다 · PR #18) — 옛 사본을 다시 적재하면 프로필 값·시각이 함께 뒤로 간다. 정상 운영(최신 아카이브만 훑음)에서는 안 일어난다 | 그렇다(재적재 시) |
| **N4** | 받아들임 | 신규 선수의 선수 페이지에서 통계 구획(`pc_stats`) **자체**가 사라지면 파서가 「원래 없음」으로 본다 — 1군 기록이 없는 선수의 실물 모양을 본 적이 없어 가를 신호가 없다(보유 0/980 · 코드 주석·런북 §7-F 에 적혀 있다) | 그렇다 |
| **N5** | P3 | `?vs=` 착지가 「이번 방문에만」 켠 대전 블록(`state.order` 에 matchup)이 뒤따르는 `save(state)` 한 번(정렬 클릭 등)으로 **저장돼 다음 방문에도 열린다** — 기준 커밋에서도 같다(구현자 탐침 · 교차 검토가 재확인) | 그렇다 |
| **N6** | P3 | 이번 방문에만 탭을 바꾸는 세 경로(해시 · 찾기 `beforematch` · W7 의 通算 착지)가 `aria-selected` 만 바꾸고 **로빙 tabindex** 는 안 바꾼다 — 키보드로 들어가면 通算이 선택됐는데 초점은 今季 탭에 간다 | 아니다(초점이 어긋나 보인다) |
