...
/**
 * ⚠**득점 유도가 실패하면 타석 로그를 만들지 않는다**(2026-08-24 · 감사 P3 #8).
 *
 * 전에는 `runsForCompleted[i] ?? 0` 이 **그 경기 전 타석을 무득점으로** 만들었다 —
 * 화면은 그것을 정상값으로 그리고, 박스 합계와 어긋난 채 조용히 남는다.
 * ⚠**0 은 「모른다」가 아니다**(M11). **틀린 값보다 없는 값이 낫다.**
 *
 * ⚠**「안 줬다」와 「줬는데 비었다」를 가른다** — 앞은 「득점을 안 본다」는 뜻이고
 * (이 파일의 다른 시험들이 그렇게 부른다), 뒤는 **유도가 실패했다**는 뜻이다.
 * ⚠**지금 걸리는 것은 0건**이다(실측: 타석이 있는 7,518경기 전수에서 「파생 0 · 박스 >0」 0건).
 */
test("⚠득점 목록이 빈 채로 오면 타석을 만들지 않는다 — 0 으로 메우지 않는다", () => {
  const b = box([batter("B1", "타자", ["左越本①", "三 振"])]);
  const events = [
    ev(1, "B1", "P1", "レフトソロホームラン（打点1）", { inning: 1, outsBefore: 1, bases: "12" }),
    ev(2, "B1", "P2", "空振り三振", { inning: 4, outsBefore: 2, bases: "3" }),
  ...
  const r = alignPaEvents("g1", b, events, []);
  assert.equal(r.events.length, 0, "빈 득점 목록인데 타석을 만들었다 — 전부 0 득점이 된다");
});

/** ⚠**안 주는 것은 막지 않는다** — 짝짓기만 보는 호출이 실재한다(이 파일의 다른 시험들) */
test("⚠득점 목록을 아예 안 주면 평소대로 짝짓는다", () => {
  const b = box([batter("B1", "타자", ["左越本①"])]);
  const events = [ev(1, "B1", "P1", "レフトソロホームラン（打点1）", { inning: 1, outsBefore: 1, bases: "12" })];
  const r = alignPaEvents("g1", b, events);
  assert.equal(r.events.length, 1, "안 줬는데 막혔다 — 「안 줬다」와 「줬는데 비었다」가 안 갈린다");
});

/** ⚠**성립 타석이 하나도 없으면 빈 목록이 맞다** — 그건 실패가 아니다 */
test("⚠성립 타석이 없으면 빈 득점 목록이어도 막지 않는다", () => {
  const b = box([batter("B1", "타자", [])]);
  const r = alignPaEvents("g1", b, [], []);
  assert.equal(r.events.length, 0);
  assert.equal(r.quarantine.length, 0, "정상 상태를 격리로 셌다");
});
