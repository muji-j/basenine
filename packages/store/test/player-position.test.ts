...
/**
 * **볼카운트 결측 0건** — カウント別의 통산 전환이 이 사실 위에 서 있다.
 *
 * ⚠**「N/N 실측」이라고 적지 않는다.** 그 총계는 **진행 중 시즌을 포함해 매일 늘어나고**,
 * 실제로 그렇게 적었다가 두 번 틀렸다(2026-08-27): 자라는 수인 데다 **손으로 더한 값 자체가
 * 틀렸다**(482,563 이라고 적었는데 정규 타석은 552,563 이다).
 * → **전칭 명제로 적고 시험으로 지킨다.** 「결측 0건」은 자라지 않고, 틀리면 바로 거짓이 된다.
 *
 * ⚠**이것이 무너지면 통산 카운트별이 「미수집을 0으로 메우는」 값이 된다**(M11) —
 * 시즌 쪽은 그 시즌만 보므로 티가 안 나고, **통산에서만** 조용히 분모가 부푼다.
 */
test("⚠볼카운트에 결측이 없다 — 통산 カウント別이 이 위에 서 있다", { skip }, () => {
  const db = openDb(DB, NOW);
  try {
    const r = db.raw
      .prepare(
        "SELECT COUNT(*) AS total, SUM(CASE WHEN ball_count IS NULL THEN 1 ELSE 0 END) AS missing FROM pa_event",
      )
      .get() as unknown as { total: number; missing: number };
    console.log(`  · 볼카운트 결측 ${r.missing} / 타석 ${r.total}`);
    assert.ok(r.total > 0, "타석이 0건이다 — 이 시험이 공회전한다");
    assert.equal(
      r.missing,
      0,
      `볼카운트가 없는 타석이 ${r.missing}건이다.\n`
        + "⚠**통산 カウント別이 그만큼 조용히 분모를 잃는다**(M11) — 시즌 쪽은 티가 안 난다.\n"
        + "  스키마는 NULL 을 허용하므로(마이그레이션 008) 소급 시즌에서 나올 수 있다.",
    );
  } finally {
    db.close();
  }
