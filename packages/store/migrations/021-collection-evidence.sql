...
CREATE TABLE schedule_month (
  ...
  --   빈 날짜 행까지 세면 개막 전 페이지가 3/1 부터 빈 행을 실을 때 3월부터 멈춘다(4라운드 F2) — 빈 행이 빠져도 잃는 증거는 없다.
  --   NULL = 기준선 없음(첫 사본 · 내용 없는 달 · 사람이 초기화함) — 그때는 사실(경기 행 · 치러짐 표시)로만 막는다
  -- ⚠이 칸은 머지 전에 더했다 — 운영 DB 에 021 이 적용된 적이 없다(옛 021 을 적용한 로컬 DB 는 적재기가 `no such column` 으로 멈춘다)
  first_content TEXT,
