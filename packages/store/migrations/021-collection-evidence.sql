...
CREATE TABLE schedule_month (
  ...
  -- ⚠받아들인 사본의 **내용이 있는 날짜 행 중 날짜 순서로 가장 이른 날**(`YYYY-MM-DD` · 경기 · 예정 표기 · 구단 아님) — 잘림의 기준선이다.
  --   새 사본이 이보다 늦게 시작하는데 이 날이 이미 왔으면 잘린 것이다(설계 D5 · 2026-09-11 3·4라운드 재검토).
  --   다른 표의 관측(앞으로의 경기 · 予告先発)으로 추측하면 予告先発이 안 지워져 개막 연기 뒤 적재기가 시즌 내내 멈췄다(3라운드).
  --   빈 날짜 행까지 세면 개막 전 페이지가 3/1 부터 빈 행을 실을 때 3월부터 멈춘다(4라운드 F2). ⚠대가: 전 사본에서 비어 있던 날이 빠진 사본은
  --   받아들이므로 그 사이 그 빈 날에 생긴 경기 · 첫 내용일이 오기 전에 받은 사본의 첫 내용일을 넘는 잘림은 조용하다(5라운드 D1 · 설계 §5).
  --   첫 내용일이 온 뒤의 잘림은 멈춘다(6라운드 3차). 기준선은 날짜 순서로 가장 이른 내용 날짜다(문서 순서 아님 · 5라운드 V1).
  --   NULL = 기준선 없음(첫 사본 · 내용 없는 달 · 사람이 초기화함) — 그때는 사실(경기 행 · 치러짐 표시)로만 막는다
  -- ⚠이 칸은 머지 전에 더했다 — 운영 DB 에 021 이 적용된 적이 없다(옛 021 을 적용한 로컬 DB 는 적재기가 `table schedule_month has no column named first_content` 로 exit 1 한다 · 5라운드 2차 실행)
  first_content TEXT,
