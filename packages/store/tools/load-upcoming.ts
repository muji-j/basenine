 ...
 * ⚠**달을 바꾸기 전에 그 사본이 온전한지 본다** — 날짜 완결성 · 잘림의 근거(사실 · 전 사본에서 날짜 순서로 가장 이른 내용 날짜)는 `missingDays`.
 * ⚠**그 달 파일은 그 달 날짜 행만 쓴다** — 섞인 다른 달 행은 지울 파일이 없어 헛 치러짐 표시 · 재실행 충돌을 만든다(6라운드 재검토 2차 F3 · 아래 루프).
 *
 ...
 * ⑵ **그 달 페이지 자신의 이력**: 받아들인 전 사본의 내용이 있는 날짜 행 중 **날짜 순서로 가장 이른 날**(`schedule_month.first_content` · 경기 · 예정 표기 · 구단 아님).
 *    ⚠빈 행은 세지 않는다 — 개막 전 페이지가 3/1 부터 빈 행을 싣는다면 개막일부터 싣는 뒤 페이지를 멈춘다(4라운드 F2).
 *    ⚠**맞교환이다**(5라운드 2차 D1 · 실행 재현): 전 사본에서 비어 있던 앞쪽 날이 빠진 사본은 받아들이므로 **조용한 경우가 둘** 생긴다 —
 *    ㈎ 그 사이 그 빈 날에 경기가 새로 생겼다(적재기가 멈춘 기간의 振替) ㈏ **첫 내용일이 오기 전**(앞쪽 휴식 동안)에 받은 사본이 첫 내용일을 넘어
 *    잘려 있다(옛 기준선 = 첫 날짜 행이면 그 달 1일이 온 뒤로 멈췄다). ⚠첫 내용일이 **이미 온 뒤**의 잘림은 멈춘다(6라운드 3차 실행 재현).
 *    창은 앞쪽 휴식이 긴 달일수록 넓다(실물 11개월 · 2021-08 은 12일 · 설계 §5).
...
/** 파일의 달과 다른 날짜의 경기 행 — 적재하지 않고 요약에 찍는다(6라운드 재검토 2차 F3) */
const foreignRows: string[] = [];
/**
...
db.transaction(() => {
  ...
  for (const f of files) {
    ...
    for (const g of r.games) {
      /**
       * ⚠⚠**그 달 파일은 그 달 행만 쓴다**(6라운드 재검토 2차 F3 · 실행 재현). 교체가 달 단위라 다른 달 날짜 행을 넣으면 **그 행을 지울 파일이 없다** —
       * ⑴ 제 달 파일과 카드 번호(`playedSeen` · `seqOf` 는 파일끼리 공유)가 겹쳐 짝 없는 치러짐 표시가 남고(A 헛경보 · 어느 방향이든)
       * ⑵ 제 달 파일이 없으면 **같은 입력의 두 번째 실행이 UNIQUE 충돌로 멈춘다**(M5 · 배포가 매일 막힌다).
       * ⚠시즌 단위로 지우던 옛 적재기에는 없던 문제다 — 달 단위 교체가 만들었다. ⚠버리지 않고 **세어 요약에 날짜를 찍는다**(실물 75장 0행).
       * 그 날의 행은 제 달 파일이 싣는다 — 날짜 완결성 · 기준선도 이미 그 달 키만 본다.
       */
      if (g.date.slice(5, 7) !== mm) {
        foreignRows.push(g.date);
        continue;
      }
      if (g.played) {
        playedRows += 1;
...
console.log(
  `일정 ${months}개월분 · 앞으로의 경기 ${kept}건 적재 · 치러진 행 ${playedRows}건 제외` +
    ...
    (foreignRows.length === 0
      ? ""
      : ` · ⚠다른 달 날짜 행 ${foreignRows.length}건 제외(날짜: ${[...new Set(foreignRows)].slice(0, 8).join(" · ")}${new Set(foreignRows).size > 8 ? " …" : ""})`),
);
/**
