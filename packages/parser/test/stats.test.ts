 ...
 */
test("⚠서로 다른 기준일이 둘 이상이면 던진다 — 고르는 순간 추측이 된다", () => {
 ...
 */
test("⚠같은 기준일이 두 번 나오는 것은 던지지 않는다 — 고를 것이 없다", () => {
  const html = "<p>2026年9月7日 現在</p><footer>2026年9月7日 現在</footer>";
  assert.equal(publishedAsOf(html), "2026-09-07");
});

/**
 * ⚠**「現在」 앵커가 이 파서의 전부다 — 그런데 그것을 지워도 시험이 전부 초록이었다**
 * (2026-09-09 · 2차 검토 F6). 아카이브 120장 전수에서 `◯年◯月◯日` 이 나오는 자리가
 * 곧 기준일 배너뿐이라, 앵커를 빼도 같은 결과가 나온다 — **살아남는 뮤테이션**이다.
 * 이 저장소는 같은 파일에서 이미 한 번 겪었다(`106.1` 뮤테이션 생존).
 * → **앵커 없는 날짜를 안 집는다**를 직접 단언한다.
 */
test("⚠「現在」가 없는 날짜는 기준일이 아니다 — 앵커를 지우면 이 시험이 붉어진다", () => {
  assert.equal(publishedAsOf("<p>2026年9月7日</p>"), null);
  assert.equal(publishedAsOf("<title>2026年度 公式戦 2026年9月7日</title>"), null);
});

/**
 * ⚠**모양만 맞고 뜻이 없는 날짜를 흘리지 않는다**(M7). 이 값은 문자열 비교로 쓰이므로
 * `2026-13-45` 가 들어가면 어떤 경기일보다도 크게 잡혀 **「안 자름」으로 조용히 흘러간다.**
 */
test("⚠날짜가 아닌 것은 던진다 — 문자열 비교라 조용히 「안 자름」이 된다", () => {
  assert.throws(() => publishedAsOf("<p>2026年13月45日 現在</p>"), StatsParseError);
});
