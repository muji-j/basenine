...
export type BoxScore =
  ...
  /**
   * **아직 끝나지 않았다** — 표는 있지만 이 경기의 기록은 **잠정**이다(M9).
   *
   * ⚠**「확정과 잠정을 타입 수준에서 구분한다」가 M9다.** 같은 `played` 로 돌려주면
   * 부르는 쪽이 구별할 방법이 없고, 진행 중인 경기의 3타수 1안타가 **그 경기의 최종 성적**으로
   * 저장된다. 그래서 상태를 하나 늘린다 — 불리언 필드로 두면 무시하기 쉽다.
   *
   * ⚠**여기 오는 것은 「진행 중」만이 아니다.** 「표는 있는데 종료 표시가 없다」의 전부다.
   * 표시가 바뀌었을 때도 여기로 오며, 그때 우리는 **저장하지 않는 쪽**으로 틀린다(안전측).
   */
  | { status: "inProgress" };
...
/**
 * **이 경기가 끝났는가** — 소스가 직접 말한다.
 *
 * ⚠**시각으로 추측하지 않는다.** 지금까지는 「자정을 넘겨 어제 것을 받으면 확실히 끝나 있다」는
 * **시간 규칙**으로 M9 를 지켰다. 그래서 당일 경기는 아무리 일찍 끝나도 다음날에야 올라왔다.
 * 그런데 소스에 **경기별 종료 표시가 실재한다**(2026-08-18 실측으로 확인):
 *
 * ```
 * 【試合終了】 ◇開始 18:00 ◇終了 21:05 ◇試合時間 3時間5分 ◇入場者 31,645人
 * ```
 *
 * ⚠**`試合時間` 의 유무로 본다.** 라벨 문자열은 늘어난다(試合終了 · 雨天のためコールドゲーム ·
 * グラウンド不良のためコールドゲーム …) — 목록을 손으로 관리하면 새 문구에서 조용히 틀린다.
 * `試合時間` 은 `終了 − 開始` 이라 **경기가 끝나기 전에는 존재할 수 없다.**
 *
 * ⚠**실측 근거**(외부 접속 0회 · 아카이브 전수): 2018·2019·2020·2024·2025·2026
 * 6시즌 **박스 4,900건**에서
 * · `試合時間` 있음 4,720건 = 【試合終了】4,700 + 콜드게임 20 (전부 성립한 경기)
 * · `試合時間` 없음 180건 = 中止 · ノーゲーム (전부 미성립)
 * **라벨이 없는 박스 0건 · 어긋난 건 0건.** 구형 시즌(2018~)에도 같은 형태다.
 *
 * ⚠**「진행 중인 박스」는 이 표본에 없다** — 우리는 끝난 뒤에만 받아 왔기 때문이다.
 * 그래서 이 판정은 「끝났다고 소스가 말할 때만 확정으로 본다」는 **안전측 단언**이다.
 * 틀리는 방향은 「덜 저장한다」이고, 그건 다음 실행이 메운다.
 */
function isFinished(html: string): boolean {
  return ownGameInfo(html).includes("試合時間");
}
...
export function parseBoxScore(rawHtml: string): BoxScore {
  ...
  /**
   * ⚠**끝났다고 소스가 말할 때만 확정으로 본다**(M9 · 2026-08-18).
   * 여기가 없으면 진행 중인 경기의 3타수 1안타가 **그 경기의 최종 성적**으로 저장된다.
   * 그러면 화면은 그것을 확정 기록과 똑같은 얼굴로 내고, 우리는 「버그인가 정정인가」를 구별할 수 없다.
   */
  if (!isFinished(html)) return { status: "inProgress" };
