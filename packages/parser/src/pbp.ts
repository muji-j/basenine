 ...
 *
 * ⚠**`規則違反アウト`(반칙 타구 등)도 여기다.** 접촉은 있었지만 **원문에 방향도 종류도 없다** —
 * 「모른다」를 「뜬공」으로 만들지 않는다(M11). 2022 아카이브에서 나왔고
 * (`2022/0723/t-db-15` 嶺井), 빠져 있으면 `battedBalls` 가 임계값 0으로 던져 **빌드가 멈춘다.**
 * ⚠**어휘표가 둘이다** — 여기(타석 로그)와 `tokens.ts`(박스 결과 셀 `違反`).
 * 한쪽만 고치면 절반만 고친 것이다.
 */
const NO_BALL = [
