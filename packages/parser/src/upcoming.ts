...
export interface ScheduleRows {
   ...
   *   「이미 온 날이 빠졌다」로 멈춘다(4라운드 재검토 2차 F2). ⚠대가가 있다(5라운드 2차 D1) — **전 사본에서 비어 있던 날**이 빠진 것은 받아들이므로,
   *   그 사이 그 빈 날에 경기가 새로 생겼거나 **첫 내용일이 오기 전**에 받은 사본이 첫 내용일을 넘어 잘려 있으면 못 잡는다(첫 내용일이 온 뒤의 잘림은 멈춘다 · 설계 §5).
   *   ⚠순서는 페이지에 나온 순서다 — 적재기가 날짜 순서로 다시 고른다(5라운드 V1).
   */
  contentKeys: string[];
}
