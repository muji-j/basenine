 ...
 * npb.jp 드래프트 페이지의 표지.
 *
 * ⚠⚠**`開催要項` 을 표지로 쓰지 마라 — 실측이 정반대를 말한다.** 2026 톱에 그 말이 있어서
 * 「개최 전 표지」로 삼고 싶어지는데, 실측 **13장 중 4장에만** 있고 **2001·2006 연도 톱에는
 * 없다**(그 두 해는 슬러그가 **12개씩 정상**이다). 그걸 표지로 쓰면 **2001 년의 마크업
 * 붕괴가 「아직 안 열렸다」로 읽힌다** — 놓치면 안 되는 쪽을 정확히 놓친다.
 *
 * `page_draft` 는 실물 드래프트 페이지 **12장 중 12장**(연도 톱 6 + 구단별 6)에 있고,
 * 실물 404 본문(nginx · 162바이트)에는 **없다**. ⚠구단별 페이지도 이 표지를 갖는다 —
 * 이건 「드래프트 섹션의 페이지인가」를 보는 것이지 「연도 톱인가」가 아니다.
 */
