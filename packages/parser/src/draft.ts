 ...
 *
 * ⚠**하드코딩 금지.** 슬러그는 **연도의 함수**다 — 실물 실측(2026-09-05):
 * | 연도 | 오릭스 | 요코하마 | 그 밖 |
 ...
 * @throws {DraftParseError} 슬러그가 0건일 때 · **`href` 의 `draftlist_` 꼬리가
 *   `{소문자}.html` 이 아닐 때**(아래).
 *
 *   ⚠**0건이 「개최 전」일 수도 있다.** 실측: 2026 연도 톱(개최 2개월 전)은
 *   `開催要項`·`ニュース` 뿐이고 `draftlist_*` 가 **0건**이다 — 그건 결측이 아니라
 *   **아직 없음**이고, `draftlist_*` 는 **개최 당일에 생긴다**(조사 문서 §7).
 *   ⚠**그래도 던진다**: 파서는 「아직 없음」과 「마크업이 바뀜」을 **구별할 수 없고**,
 *   빈 배열로 흘리면 진짜로 바뀐 날에도 똑같이 조용하다. **어느 쪽인지는 호출자가 정한다.**
 */
export function parseDraftTeamSlugs(html: string): string[] {
  ...
  if (slugs.size === 0) {
    throw new DraftParseError(
      "draftlist 링크를 못 찾았다 — 빈 배열로 흘리지 않는다(M7). ⚠개최 전 연도일 수 있다(판정은 호출자)",
      `html ${html.length}자`,
    );
  }
  return [...slugs].sort();
