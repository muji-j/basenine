 ...
 *
 * ⚠**이 저장소는 같은 병으로 이미 한 번 데었다** — `0788df2`
...
export interface PublishedSample extends Sample {
   ...
   * 시즌 중에 받은 것일 수 있다.** 선수 페이지는 경기 다음날 새벽에도 아직 그 경기를 안 싣는다
   * (실측: 08-19 01:05 JST 취득분이 08-18 경기를 안 담았다) — 그래서 시즌 최종일에 뛴 선수는
   * **그 경기가 빠진 사본**을 든 채로 시즌 종료를 맞는다.
   * ⚠**선정식은 그 사이 고쳤다**(`store/tools/emit-stale-player-ids.ts` · 2026-08-20).
   * 이제 날짜가 아니라 **출장량이 모자란가**로 판정하므로 그 사본은 다음 실행에서 다시 받힌다.
   * 그래도 이 필드는 **없앨 수 없다** — 다시 받히기 **전까지의 창**(하루~이틀)과,
   * 페이지가 사라져 끝내 못 고치는 선수가 남는다. 그때 「완결이니 같아야 한다」고 우기면
   * 그건 우리 집계의 결함이 아닌데 결함으로 잡힌다.
