...
/**
 * ⚠**바로 위 픽스처는 원값이 `−1` 이라 경계를 안 밟는다**(2026-08-20 최종 검토 ①).
 *
 * 그 수정이 설치한 보호막은 `magic > 0` 이고, `> 0` 과 `>= 0` 은 **원값이 정확히 `0` 일 때만** 갈린다.
 * 그런데 위 픽스처의 원값은 `−1` 이라 둘 다 `null` 을 낸다 —
 * 실측(2026-08-20 리뷰어): **`>= 0` 으로 바꿔도 1417/1417 이 그대로 초록**이었다.
 * 즉 **이 브랜치가 두 번 고친 Critical 의 경계를 아무도 재고 있지 않았다.**
 *
 * 이 픽스처는 위와 **`s` 의 승수 하나만** 다르다(56 → 57). 그 하나가 원값을 `−1` 에서 `0` 으로 옮긴다:
 * ```
 * t 73-47- 8 (128경기 · 잔여 15)  최선 88/135 = .652  최악 73/135 = .541   ← 선두 · 자력 생존
 * s 57-55-16 (128경기 · 잔여 15)  최선 72/127 = .567                       ← .567 > .541 이라 **안 죽었다**
 * 나머지 4팀 50-78-0             최선 65/143 = .455                       ← 전부 소멸
 * 승수식 원값 = 57 + 15 − 73 + 1 = 0
 * ```
 * 실측(2026-08-20 프로브 · 현행 코드): `basis confirmed` · `disagreed []` · `t.magic = null`.
 * `>= 0` 뮤턴트에서는 `t.magic = 0` 이 되어 화면이 「優勝が決まりました」라고 말한다 —
 * **`s` 가 아직 안 죽었는데** 우승을 단정하는 것이다.
 *
 * ⚠**위 `−1` 픽스처 바로 옆에 둔다** — 둘이 함께 「음수」와 「정확히 0」을 못 박는다.
 * ⚠**뮤테이션 확인(2026-08-20 실측)**: `magic > 0` → `magic >= 0` 으로 바꾸면
 * `race.test.ts` **32본 중 이 한 본만** 떨어진다(FAIL 1 · ERROR 0).
 */
test("⚠승수식 원값이 정확히 0 이어도 매직은 null 이다 — `> 0` 과 `>= 0` 이 갈리는 유일한 점", () => {
  const pp = pairs(22, 3); // 위 픽스처와 같은 대전표 — 팀당 128경기 · 잔여 15
  // ⚠**`s` 의 승수만 하나 늘렸다**(56 → 57). 그 하나가 원값을 `−1` 에서 `0` 으로 옮긴다
  const over: Record<string, Over> = { t: { w: 73, t: 8 }, s: { w: 57, t: 16 } };
  for (const c of CENTRAL) if (c !== "t" && c !== "s") over[c] = { w: 50 };
  const r = seasonRace({ season: 2026, teams: teams(pp, over), leagueOf, playedPairs: pp });
  assert.equal(r.basis, "confirmed");
  assert.equal(r.teams.get("t")!.remaining, 15);
  // ⑴ 점등 조건은 성립한다 — 이게 깨지면 아래 `null` 은 「매직이 안 켜졌다」라는 다른 이유로 통과한다
  assert.equal(r.teams.get("t")!.selfPossible, true, "선두의 자력이 죽었다 — 이 시험의 전제가 깨졌다");
  for (const c of CENTRAL) {
    if (c === "t") continue;
    assert.equal(r.teams.get(c)!.selfPossible, false, `${c}: 자력이 살아 있으면 매직이 애초에 안 켜진다`);
  }
  // ⑵ 그런데 `s` 는 아직 안 죽었다 — 「전원 소멸 → 0」 가지도 성립하지 않는다
  assert.equal(r.teams.get("s")!.eliminated, false, "s 가 소멸이면 이 시험이 재려는 상태가 아니다");
  // ⑶ 그러므로 원값이 `0` 이어도 매직은 `null` 이다. **`>= 0` 이면 여기서 `0` 이 나온다**
  assert.equal(
    r.teams.get("t")!.magic,
    null,
    "승수식 원값 0 을 그대로 내보냈다 — 라이벌이 살아 있는데 「優勝が決まりました」가 된다",
  );
});
