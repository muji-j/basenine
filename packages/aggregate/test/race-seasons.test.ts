...
const PENNANT: ReadonlyMap<number, Readonly<Record<(typeof LEAGUES)[number], Champion>>> = new Map([
   ...
   * 阪神 / ソフトバンク.
   *
   * ⚠**이 두 줄만 출처가 다르다.** 나머지 14건은 기억에서 적었지만 여기는 확신이 부족했고
   * (阪神이 85승인지 86승인지 갈렸다) **추측 대신 비워 뒀다가**,
   * `https://npb.jp/bis/2025/standings/` 의 최종 순위를 직접 읽어 채웠다(2026-08-19).
   * 실제로 **85승**이었다 — 추측했으면 절반의 확률로 이 표 전체가 거짓이 됐다.
   * ⚠**DB 값을 옮겨 적지 않았다.** 그러면 자기 검산이 되어 이 표가 아무것도 못 잰다.
   * 검산: 85+54+4 = 143 · 87+52+4 = 143 (규정 경기수와 일치).
   */
  [2025, { central: { code: "t", wlt: [85, 54, 4] }, pacific: { code: "h", wlt: [87, 52, 4] } }],
]);
 ...
 *
 * ⚠**매직의 「값」도 이제 박는다**(2026-08-19 매직 클램프 수정). 이전 판은 여기서 값을 일부러
 * 못 박았다 — 실측으로 **2021 센트럴의 우승팀 ヤクルト(`s`, 73-52-18 · .584)의 매직이 잔여 0인데
 * `5`** 였기 때문이다. 2위 阪神(`t`)이 **77승**으로 우승팀보다 승수가 많고(무승부 18 대 10),
 * 매직 식은 관례대로 승수식(`2위 최대 승수 − 내 승수 + 1`)인데 순위 판정은 승률식이라 둘이 갈렸다 —
 * 이미 우승한 팀에 매직 5 가 남는 상태였다(클램프 주석 「`0` = 이미 우승 확정」과 모순).
 * `race.ts` 가 **「리그 내 다른 팀 전부 `eliminated === true` 면 `magic = 0`」**을 증명해서
 * 접도록 고쳤으므로(승수식 자체는 안 바꾼다), 이제 단독 우승은 반드시 `0` 이어야 한다.
 */
    ...
    for (const league of LEAGUES) {
      ...
      for (const x of s.teams.filter((t) => leagueOf(t.teamCode) === league)) {
        ...
        if (!champs.includes(x.teamCode)) {
          ...
          // ⚠**완결 시즌의 단독 우승은 반드시 매직 0 이다**(2026-08-19 매직 클램프 수정).
          // 승수식·승률식이 갈려도(2021 센트럴이 실측 반례였다) 우승 확정 팀에는 0 이 나가야 한다.
          assert.equal(
            r.magic,
            0,
            `${s.season} ${league} ${x.teamCode}: 단독 우승인데 매직이 ${r.magic} 이다 — 이미 우승 확정이면 0 이어야 한다`,
          );
          lit += 1;
        } else {
          assert.equal(
