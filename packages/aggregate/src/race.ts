...
export function seasonRace(o: {
  ...
  if (confirmed) {
    for (const me of o.teams) {
      ...
      /**
       * ⚠**초기값을 `0` 으로 두지 않는다.** 옛 판은 `0` 으로 시작해 `Math.max` 로만 누적했고,
       * 그래서 「`magic >= 0`」이 **어떤 식을 넣어도 참인 단언**이 되어 시험이 뮤테이션을 하나도
       * 못 잡았다(실측 2026-08-19: 그 픽스처의 원식은 `20 + 8 − 130 + 1 = −101` 인데 값은 0 이었다).
       * `-Infinity` 로 두면 아래 판정이 **원값을 그대로 본다** — `> 0` 인지 아닌지가 실제 갈림길이 된다.
       * (`others.length === 0` 은 위에서 걸렀으므로 이 루프는 반드시 한 번 이상 돈다 = `-Infinity` 가
       *  아래로 새지 않는다. 그 가지를 지우면 `-Infinity > 0` 이 거짓이라 `null` 이 나간다 —
       *  옛 클램프 시절에는 같은 자리에서 **`0`(우승 확정)**이 나갔다.)
       */
      let magic = -Infinity;
