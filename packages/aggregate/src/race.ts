...
export interface TeamRace {
  ...
  /**
   * 매직 넘버(승수식). **`0` 은 「이미 우승 확정」이고, 그 외에는 반드시 양수다.**
   *
   * `null` 인 경우가 **둘**이다(2026-08-19 검토 지적 a) — 화면은 둘 다 「매직 없음」으로 다루면 된다.
   * 1. **점등 조건 미충족** — 같은 리그의 다른 팀 중 자력우승이 살아 있는 팀이 있다.
   * 2. **점등은 했는데 승수식으로 표현할 수 없다** — 원값이 `0` 이하인데 우승은 아직 확정이 아니다.
   *    무승부가 많으면 승수식(매직의 관례)과 승률식(NPB 순위)이 갈려서 실제로 일어난다.
   *    ⚠**여기서 `0` 을 쓰면 「우승 확정」이라는 단정이 된다** — 그래서 `null` 이다(M11).
   */
  magic: number | null;
...
export function seasonRace(o: {
  ...
  if (confirmed) {
    for (const me of o.teams) {
       ...
       *   ⚠**경로 정정(2026-08-19 검토 지적 a)**: 예전에는 「`Math.max` 초기값 `-Infinity` 가
       *   클램프를 타고 `0` 이 된다」였는데 **클램프를 지웠으므로 그 경로는 없다.** 그래도 가지는
       *   그대로 필요하다 — 아래 `rivalsAllGone` 이 **빈 배열에서 공허참**이라 여전히 `0` 이 나간다.
       *   (클램프가 없는 지금은 그 가지를 지웠을 때 나오는 값이 `0` 하나뿐이라 **더** 위험하다:
       *    옛 판에서는 원값이 양수면 그 값이 나갈 여지라도 있었다.)
       ...
       * 승수식 원값의 초기값.
       *
       * ⚠**주석 정정(2026-08-20 최종 검토 ⑤ · 여기 적혀 있던 말이 실제보다 강했다).**
       * 예전에는 「`-Infinity` 로 두면 아래 판정이 **원값을 그대로 본다** — `> 0` 인지 아닌지가
       * 실제 갈림길이 된다」고 적혀 있었는데, 앞 절이 뒤 절의 **근거처럼** 읽혔다. 그건 틀렸다:
       * **오늘 갈림길을 만드는 것은 `-Infinity` 가 아니라 아래의 `> 0` 하나다.**
       * 실측(2026-08-20 뮤테이션): `-Infinity` → `0` 으로 바꿔도 **race.test.ts 32/32 가 그대로 초록**이다.
       * 등가인 이유는 산수다 — `0` 으로 시작하면 값이 `max(0, 원값)` 이 되는데,
       * 원값이 0 이하인 구간은 어차피 `> 0` 이 거짓이라 양쪽 다 `null` 로 간다.
       *
       * ⚠**그런데도 `0` 으로 되돌리지 않는다.** 등가는 `> 0` 이 `> 0` 인 동안만이다.
       * 비교가 `>= 0` 이 되는 순간 둘이 갈린다 — `-Infinity >= 0` 은 거짓(`null`)이지만
       * `0 >= 0` 은 참이라 **`0`(= 우승 확정)**이 나간다. 즉 `-Infinity` 는 갈림길이 아니라
       * **그 갈림길이 잘못 열렸을 때의 마지막 안전망**이다.
       * ⚠**그 경계에는 이제 시험이 있다** — race.test.ts 「승수식 원값이 정확히 0 이어도 매직은 null」.
       * 없던 동안에는 `>= 0` 뮤턴트가 1417/1417 을 그대로 통과했다(2026-08-20 리뷰어 실측).
       *
       * ⚠**옛 판이 남긴 교훈은 그대로다**: 초기값 `0` + `Math.max` 누적이던 시절엔
       * 「`magic >= 0`」이 **어떤 식을 넣어도 참인 단언**이라 시험이 뮤테이션을 하나도 못 잡았다
       * (실측 2026-08-19: 그 픽스처의 원식은 `20 + 8 − 130 + 1 = −101` 인데 값은 0 이었다).
       *
       * (`others.length === 0` 은 위에서 걸렀으므로 이 루프는 반드시 한 번 이상 돈다 = `-Infinity` 가
       *  아래로 새지 않는다. 그 가지를 지우면 `rivalsAllGone` 이 빈 배열에서 공허참이라 `0` 이 나간다 —
       *  막는 것은 초기값이 아니라 그 가지다.)
       */
      let magic = -Infinity;
      ...
      /**
       * ⚠**승수식과 승률식이 갈리면 「이미 우승 확정」인데 매직이 0 이 안 될 수 있다.**
       * 실측(2026-08-19 실데이터 검산 · task-3):
       * ```
       * 2021 센트럴 s: magic = 5  (remaining = 0)
       *   우승 ヤクルト  73승 52패 18무 (.584)
       *   2위  阪神      77승 56패 10무 (.579)
       * ```
       * 매직은 관례대로 **승수식**(`2위 최대 승수 − 내 승수 + 1` = `77 − 73 + 1 = 5`)인데
       * NPB 순위 판정은 **승률식**(무승부는 분모에서 빠진다)이라 무승부가 많은 해에 둘이 갈린다.
       * **승수식 자체는 바꾸지 않는다**(관례) — 대신 **우승이 확정된 순간에만** 값을 0으로 접는다.
       *
       * ⚠**이건 표시 취향이 아니라 증명되는 사실이다.** 리그 내 다른 팀이 전부 `eliminated === true`
       * 라고 하자. 그중 **최선승률이 가장 높은 팀 X\*** 를 죽인 것이 누구인가.
       * 다른 「타팀」 Y 일 수는 없다 — `Y의 최악 ≤ Y의 최선 ≤ X*의 최선` 이라
       * `Y의 최악 > X*의 최선` 이 성립할 수 없다. 따라서 **나**다.
       * 즉 `내 최악 > X*의 최선 ≥ 모든 타팀의 최선` — **우승 확정**이다. 그때 매직은 정의상 `0`.
       *
       * ⚠**부수 확인**: `eliminated === true` ⇒ `selfPossible === false` 이므로
       * (어떤 B의 최악 > 내 최선 ⇒ B의 최선 ≥ B의 최악 > 내 최선) 「전원 소멸」이면 매직의
       * **점등 조건은 이미 만족돼 있다.** 이 수정은 **점등 조건을 바꾸지 않고 값만 접는다.**
       *
       * ⚠**M11 과 충돌하지 않는다.** 「모르는데 0 을 쓰는 것」이 아니라
       * **「0 임을 증명해서 0 을 쓰는 것」**이다 — 위 증명이 그 근거다.
       * 재현 범위: 완결 8시즌 × 2리그 = 16건 중 1건(나머지는 단독 우승 14건이 애초에 0,
       * 동률 우승 1건이 `null`).
       *
       * ---
       * ⚠**그리고 그 반대 방향이 남아 있었다 — 「미확정인데 매직 0」**(2026-08-19 검토 지적 a).
       * 위 수정이 「`0` = 우승 확정」의 의미를 **강화**했기 때문에 이 오류가 더 위험해졌다.
       * 원인은 옛 `Math.max(0, magic)` 클램프다 — 승수식 원값이 0 이하인 상태를 **두 가지 다른 것**을
       * 구별하지 않고 전부 `0`(= 우승 확정)으로 접었다:
       * ⑴ 정말로 확정된 경우(위 증명이 붙는 경우) ⑵ **무승부가 많아 승수식이 승률식과 갈린 경우.**
       * ⑵ 는 아직 뒤집힌다. 실측(리뷰 프로브 · 현실형 분포): 점등 **241,384건 중 11,648건(4.8%)**이
       * 「`magic === 0` 인데 미확정」이었다. 재현 입력:
       * ```
       * t 73-47-11 (잔여 12) 선두   최악 73/132 = .553
       * s 60-56-16 (잔여 11)        최선 71/127 = .559  → .559 > .553 이라 **아직 안 죽었다**
       * 승수식 원값 = 71 − 73 + 1 = −1 → 옛 클램프가 **0**(= 우승 확정)으로 접었다
       * ```
       * ⚠무승부 11·16 은 NPB 에서 흔한 수치다(2021 센트럴이 10~20).
       *
       * → **판정 순서를 셋으로 나눈다.**
       * 1. **타팀 전원 소멸** → `0`. 위 증명이 붙는 **사실**이다.
       * 2. 아니고 원값 **`> 0`** → 그 값. 승수식이 정상적으로 의미를 갖는 구간이다.
       * 3. 아니면 **`null`** — 「승수식으로는 표현할 수 없다」. **`0` 으로 접지 않는다**(M11 —
       *    모르는 것을 아는 척하지 않는다. 여기서 `0` 은 「모름」이 아니라 **「우승 확정」이라는 단정**이다).
       *
       * ⚠**`Math.max(0, magic)` 클램프는 지웠다.** 1·3 이 그 자리를 대신한다 —
       * 원값이 음수인 경우는 1(확정이라 `0`)이거나 3(미확정이라 `null`)뿐이고,
       * 클램프가 그 둘을 뭉개는 것이 바로 이 결함이었다. 「음수를 0 으로 접는다」는 표시 방어처럼
       * 보였지만 실제로는 **가장 강한 단정을 조용히 만들어 내는 코드**였다.
       */
      const rivalsAllGone = others.every((c) => out.get(c)!.eliminated === true);
      // ⚠`0` 은 **증명된 우승 확정에만** 쓴다. 승수식으로 말할 수 없는 상태는 `null` 이지 `0` 이 아니다
      // 1. 전원 소멸 → `0`(사실) · 2. 원값 `> 0` → 그 값 · 3. 그 외 → `null`(표현 불가)
      mineRace.magic = rivalsAllGone ? 0 : magic > 0 ? magic : null;
    }
  }
