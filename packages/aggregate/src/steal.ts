 ...
 * **타석 로그가 유일한 출처**다.
 *
 * ## 대조 실측 — **완결 시즌만 센다** (2026-08-22 정정)
 *
 *   · 경기별 도루 수 대 박스스코어 `盗塁` 열: **완결 8시즌 6,875경기** 전수 **어긋남 0건**
 *   · ⚠**실질 분모는 도루가 있던 4,229경기**다 — 도루 0인 경기는 0=0 으로 자동 일치한다
 *   · **2025 정규시즌 도루 921 · 도루자 381**
 *
 * ⚠**여기 진행 중 시즌의 수를 적지 마라.** 원래 「2,395경기(2024〜2026) · 2026 도루 611」 이었는데
 * 경기가 들어올 때마다 낡아 감사 시점에 이미 **618** 이었다 — 시즌 중에는 **매일** 그렇게 된다.
 * ⚠**공표값 대조는 「그날의 사건」이다**: 2026-08-17 에 2026 도루 611 = 공표 611 ·
 * 도루자 254 = 공표 `盗塁刺` 254 로 일치했다. 그 일치는 **이력**이고, 오늘 값이 다른 것은
 * 틀려서가 아니라 **경기가 늘었기 때문**이다.
 * ⚠**이 수들은 `scripts/test/doc-figures.test.ts` 가 DB 로 다시 센다.**
 */
 ...
 * 2025 도루는 정규 921 인데 전 대회를 섞으면 **943 이 되어 22 어긋난다.**
 * ⚠**완결 시즌으로 적는다** — 진행 중 시즌으로 적으면 이 줄이 매일 낡는다.
