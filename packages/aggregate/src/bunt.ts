...
const KNOWN: ReadonlySet<Outcome> = new Set<Outcome>([
  ...
  "interferenceOut",
  /**
   * ⚠**규칙 위반 아웃도 타수다**(2022 아카이브에서 나왔다).
   *
   * ⚠**어휘를 늘리면 고칠 자리가 셋이다** — `tokens.ts`(타수 판정) · `fold.ts`(카운팅) ·
   * 여기. 앞의 둘은 타입이 잡아 주지만 **여기는 런타임 검사라 빌드가 돌 때 터진다.**
   * 실제로 그 순서로 세 번 터졌다(2026-08-17). 그게 M7 이 의도한 동작이지만,
   * 다음 사람이 놀라지 않게 적어 둔다.
   */
  "ruleViolationOut",
]);
...
/**
 * **그 투수가 그 타자를 몇 번째로 만났는가**별 성적.
 *
 * ⚠**이름이 값과 달랐다**(2026-08-24 정정 · 감사 P3 #56). 첫 줄이
 * 「타순 순회(times through the order)」라고 단언했는데, 세는 것은 **타순 순회가 아니다** —
 * 키가 `gameId|pitcherId|batterId` 라 **대타가 들어오면 투수가 3순회째여도 그 타석은 「1巡目」**이 된다.
 *
 * 실측(2025 정규 · 타석 64,178): 현행 「1巡目」 **38,435** 중 **773(2.0%)** 이
 * **투수가 이미 9타자 이상을 만난 뒤**에 나온 것이다.
 *
 * ⚠**값을 바꾸지 않고 이름을 고쳤다** — 이 저장소의 선례와 같다(SRC 를 WAR 이라 부르지 않는다).
 * 세는 양 자체는 **정당하고 오히려 더 직접적**이다: 「타자가 같은 투수를 볼수록 유리해지는가」를
 * 물을 때 **타자 본인이 몇 번째로 보는가**가 답에 더 가깝다. 거짓이었던 것은 **이름뿐**이다.
 * ⚠**타순 순회를 정말로 세려면** 타순 자리(`batting_line.batting_order`)를 붙여야 한다 — 별개 작업이다.
 *
