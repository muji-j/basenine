...
export function blendConstants(
  ...
  return {
     ...
     * ⚠**대상은 리그를 넘어 이적한 선수뿐**이다. 9시즌 전수 실측(2026-08-21):
     * 해당자 **32명** · `|Δ wRAA|` 중앙 **0.021** · 최대 **0.184**득점(2025 · 275타석).
     * ⚠**이 서술이 정본이다**(M1) — `query.ts` 의 호출부는 여기를 가리키기만 한다.
     *   예전에는 호출부가 「**정확히** 성립한다 — 증명 가능한 일반화다」라고 **반대로** 적고 있었다.
     */
    wobaWeights: {
