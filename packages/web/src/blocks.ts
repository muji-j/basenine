...
export const BLOCKS: readonly BlockMeta[] = [
  ...
  /**
   * ⚠**「巡目」は打順の一巡ではない**(2026-08-24 정정 · 감사 P3 #56).
   * 세는 것은 **그 투수가 그 타자를 몇 번째로 만났는가**다 — 대타가 들어오면
   * 투수가 3순회째여도 그 타석은 「1巡目」이 된다(실측 2025: 38,435 중 **773 = 2.0%**).
   * ⚠**설명문에 그 사실을 적는다** — 이름만 보고 다른 것을 세는 줄 알면 그게 거짓말이다.
   */
  { id: "timesthrough", name: "打順一巡", desc: "投手のみ。NPB全体の集計（この選手の記録ではありません）。⚠「巡目」は打順の一巡ではなく、その投手がその打者と何度目に対戦したかです（代打が入ると1巡目に戻ります）" },
  /**
