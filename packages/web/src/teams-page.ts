...
${note(
    ...
    "並びは**順位順**です。順位は**勝率**（勝÷（勝＋敗）・引き分けは分母に入れません・NPBの規定）で決まります。" +
      // ⚠**`決着` 은 이 사이트가 만든 말이다**(2026-08-20 이중 검토 P2). 이 화면의 카드 라벨은
      //   `<s>勝率</s>` 라 용어 버튼이 아니라서 툴팁이 없다 — 그래서 각주가 뜻을 말한다.
      //   (구단 페이지·순위표에는 `term("勝率")` 버튼이 있다.)
      "勝率の母数「**決着**」は 勝＋敗 です — 引き分けを除いた数なので、試合数より少なくなります。" +
      TIE_RULE +
