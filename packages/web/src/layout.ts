...
<div class="shell">
  ...
  <main class="main" id="main">
    ...
    <footer class="foot">
      ...
      <!-- ⚠**용어 설명으로 가는 길이 툴팁 하나뿐이었다**(2026-08-21 감사).
           툴팁은 떠 있는 상자라 「나중에 읽자」가 안 되고, 火消し率 처럼 긴 것은
           320×568 에 **원리적으로 안 들어간다**(필요 555px · 가용 480px).
           ⚠**내비가 아니라 푸터다** — 내비는 이미 7칸이고 320px 에서 줄이 넘어간다.
           푸터는 전 페이지에 있으므로 **길이 없는 화면은 0장**이 된다.
           ⚠**root 다** — 용어집은 시즌마다 만들지 않는다(log.html 과 같은 이유). -->
      用語の意味は<a href="${o.root}glossary.html">用語集</a>にまとめてあります。<br>
