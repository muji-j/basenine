 ...
 *
 * ⚠**두 벌이었고 내용이 달랐다**(2026-08-19 T7 검토 ⓓ). 순위표는 위 괄호를 밝히는데
 * 구단 목록은 그 괄호가 없었다 — **같은 사실을 두 화면이 다르게 공시**하고 있었다(M1·M3).
 * ⚠**여기 있는 이유**: 순위표는 `query.ts`(데이터)에서, 구단 목록은 렌더러에서 이 문장을 쓴다.
 * 렌더러는 DB를 몰라야 하므로 `query.ts`를 import 할 수 없다 — 둘 다 닿는 곳이 이 파일이고,
 * 이 문장이 나가는 통로(`note`)도 여기 있다.
 */
