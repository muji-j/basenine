...
function stealDetail(s: NonNullable<BattingBlockData["steal"]>): RawHtml {
  ...
  ${note(
    (both
       ...
       * ⚠**이 수는 백필하면 낡는다** — CLAUDE.md §2-2 가 「이 줄이 낡으면 판정 기준 자체가
       * 거짓이 된다」로 이미 한 번 당한 실패 모드이고, 이번엔 문서가 아니라 **유저 화면**이다.
       * → `packages/aggregate/test/steal-seasons.test.ts` 가 실DB 로 이 세 수를 고정한다.
       *   시즌을 더 넣으면 그 시험이 **떨어져서** 여기도 같이 고치게 된다.
       */
      (hasHome
