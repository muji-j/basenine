...
const BUDGET = {
  /**
   * ⚠**리듬이 아니라 기하다** — 척도(`--s*`)에 올릴 값이 아니라서 예외로 남긴다.
   *
   * `.seasons{scroll-padding-left:calc(var(--gut) + var(--pad) + 84px)}` 의 **84px** — sticky 라벨(「シーズン」)의 **폭**에서 나온 수.
   * (앞의 두 항은 띠 자신의 padding-left 다 — 2026-09-25 감사 W4 에서 더했다 · 척도 밖 값은 여전히 84px 하나다.)
   * 계단에 억지로 올리면 라벨 밑에 연도가 다시 겹친다(2026-08-18 에 고친 그 결함).
   * **라벨 폭을 재는 토큰이 생기면 그때 0 이 된다.**
   *
   * ⚠**~~2건~~ 이었다**(2026-09-08 · 2f 에서 **1건으로 줄었다**).
   * 없어진 것은 `html:has(.hjump){scroll-padding-top:calc(var(--topbar) + 52px)}` 이고,
   * **척도에 올려서 없앤 것이 아니라 그 상수가 필요 없어져서 없앴다** —
   * 2f 가 목차를 줄바꿈 + 접기로 바꾸면서 높이가 43.3~122.5px 로 움직이게 됐고,
   * **그런 바에는 정직한 상수가 없어 고정 자체를 걷어냈다**(assets.ts 의 .hjbar 문단).
   * ⚠**예산이 내려간 것은 좋은 방향이다. 올릴 때만 의심하면 된다.**
   */
  space: 1,
  fontSize: 0,
  fontWeight: 0,
