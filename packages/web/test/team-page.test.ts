...
function data(over: Partial<TeamPageData> = {}): TeamPageData {
  return {
    ...
    // ⚠**픽스처가 틀린 표기를 가르치게 하지 않는다** — 109경기의 구원 기준은 109아웃 = 36.1回 다.
    //   예전 값 `36.3回` 는 NPB 어법에 없는 수였다(2026-08-21 감사).
    pitQualifier: "先発は規定投球回 109回（NPB公式）、救援はその3分の1 36.1回（当サイトの基準でNPBのものではありません）",
    vs: [
...
test("⚠잔여 0 의 동률을 현재형으로 말하지 않는다", () => {
  ...
  // ⚠**완전 일치로 「リーグに」를 고정한다**(2026-08-19 재검토 — fe04202 이 놓쳤고 1차 검토도
 ...
 * 실측: 구단 페이지 **108/108 장** · 선수명 셀 **923개**(연속 59 + 근접 864)가 **전부 링크 아님**.
 * ⚠**분모를 두 번 틀렸다.** 처음 지적은 1,547(1.68배 부풀림), 이 시험의 첫 주석은 833(90 적음)이었다.
 *   833 은 **이름 사전으로 본문을 훑는** 스캐너의 값이라 1글자 이름과 「같은 문자종이 이어지는 자리」를 뺐다.
 *   이 질문의 옳은 분모는 **셀 수**다 — `<td class="l">` 를 세면 923 이고, 고친 뒤 923/923 이 링크다.
 *
