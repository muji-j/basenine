...
test("⚠도루자를 세지 못했으면 未集計다 — 0으로 때우면 성공률이 1.000이 된다", () => {
  ...
  /**
   * ⚠**페이지 전체에서 `"1.000<span"` 을 찾고 있었다**(2026-08-31 정정). 그건 **이 항목이
   * 아니라 화면 어딘가에 1.000 이 있는가**를 재는 줄이었고, スプリット 의 OPS 에 분모를 붙이자
   * (같은 픽스처의 OPS 가 정확히 1.000 이다) **관계없는 변경에 붉어졌다.**
   * ⚠**시험을 통과시키려고 픽스처의 값을 바꾸지 않았다** — 그러면 다음에 다른 항목이 우연히
   * 1.000 이 될 때 같은 일이 또 난다. **재는 자리를 그 항목으로 좁히는 것**이 답이다.
   */
  // ⚠라벨은 `term()` 이 `<button>` 으로 감싼다 — `</dt>` 까지 건너뛰고 그 다음 `<dd>` 를 본다
  const cell = /盗塁成功率[\s\S]{0,40}?<\/dt>\s*<dd[^>]*>([\s\S]*?)<\/dd>/.exec(out)?.[1] ?? "";
  assert.notEqual(cell, "", "盗塁成功率 항목 자체를 못 찾았다 — 이 시험이 공회전한다");
  assert.ok(!cell.includes("1.000"), `세지 못했는데 성공률 1.000을 냈다: ${cell}`);
