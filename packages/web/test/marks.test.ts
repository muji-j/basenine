...
/**
 * ⚠**「出塁」 축의 단위가 `打席` 이었는데 그건 사실이 아니었다**(2026-08-20).
 * 出塁率의 분모는 `打数+四球+死球+犠飛`(= 打席 − 犠打)라 옆의 打席 열과 다른 수인데
 * 이름이 같아서, 배포물에서 같은 화면에 「打席」이 두 개 떴다.
 * 이 픽스처는 둘을 442 로 같게 잡고 있어 **수로는 구별할 수 없다** — 그래서 여기서 재는 것은
 * 단위 문자열이고, 수의 차이는 `den-units.test.ts` 가 배포물에서 잰다.
 */
test("⚠축마다 자기 분모를 들고 다닌다 — 打率는 打数, 出塁는 出塁機会다", () => {
  ...
  assert.deepEqual(
    axes.map((a) => a.sample),
    ["382打数", "442出塁機会", "382打数", "442打席", "442打席"],
  );
  // 분모가 비어 있으면 M2가 무너진다
