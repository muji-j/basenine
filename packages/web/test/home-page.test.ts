 ...
 * ⚠**한 줄은 한 출처여야 한다.**
 *
 * 처음에는 「통산」을 NPB 공표치에서, 「今季」를 우리 경기 데이터에서 가져왔다.
 * 두 출처의 **기준일이 다르다** — 실측(2026-08-17): 선수 페이지의 年度別成績은 **8/14까지**를
 * 반영하는데 우리 경기 데이터는 **8/16까지**다. 그래서 화면에 `통산 90 · 今季 13` 이 서고,
 * **90 − 13 = 77** 인데 그 선수의 작년까지 통산은 **78** 이었다 —
 * **한 줄 안에서 뺄셈이 성립하지 않았고**, 마디까지 남은 수도 하루치만큼 틀렸다.
 *
 * 여기서 재는 것은 **화면이 그 사실을 말하는가**다. 계산 자체는 조회 계층에서
 * 같은 표를 쓰도록 고쳤다(같은 `career_*` 에서 합계와 그 해 분을 함께 뽑는다).
 */
test("⚠통산과 今季가 같은 출처라고 화면이 말한다", () => {
  const out = renderHomePage(data(), context());
  assert.match(out, /同じ年度別成績から取っています/, "같은 출처라고 말하지 않는다");
  assert.match(out, /引き算が合わなくなる/, "왜 그래야 하는지 말하지 않는다");
  // 우리 경기 데이터와는 기준일이 다를 수 있다는 것도 말한다
  assert.match(out, /日付の基準が違うことがあります/, "기준일이 다를 수 있다는 말이 없다");
});
