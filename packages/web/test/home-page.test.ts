...
/**
 * ⚠**통산 표는 두 출처를 이어 붙인 것이다**(M4).
 *
 * 지난 시즌까지는 NPB 공표치, **올해는 우리 집계**다. NPB 선수 페이지가 우리보다
 * **며칠 늦게** 갱신되기 때문이고(실측 2026-08-17: 2026년 타격 617명 중 182명·투구 358명 중
 * 90명이 우리 쪽이 크고, **NPB 쪽이 큰 경우는 0명**), **끝난 시즌에서는 두 값이 완전히 일치**한다
 * (2023~2025 타격 1,777건·투구 983건, 어긋남 0). 그 실측이 있었기에 이어 붙일 수 있다.
 *
 * ⚠**화면이 그 구성을 말해야 한다** — 근거 없이 「섞었다」고만 하면 값을 믿을 수 없다.
 */
test("⚠通算 블록이 「올해는 우리 집계」와 그 근거를 말한다(M4)", async () => {
  ...
  assert.match(out, /今シーズン）は当サイトの集計/, "올해가 우리 집계라는 말이 없다");
  assert.match(out, /数日遅れて/, "왜 그렇게 하는지 말하지 않는다");
  assert.match(out, /終わったシーズンでは両者が完全に一致/, "이어 붙여도 되는 근거가 없다");
  assert.match(out, /通算は当サイトが足した値/, "우리가 더했다는 말이 없다");
  // 줄마다 출처 표시가 있다
  assert.match(out, /<s>当サイト<\/s>/, "우리 집계 행에 표시가 없다");
  // ⚠**시즌 수는 행 수가 아니다** — 픽스처에 2025년 두 줄(이적)이 들어 있다
  assert.match(out, /2シーズン/, "이적한 해를 두 시즌으로 셌다");
});

/**
 * ⚠**盗塁刺는 박스스코어에 없다**(CLAUDE.md §2-2). 올해 행은 우리 집계라 그 값이 없다 —
 * **0으로 적으면 「올해는 한 번도 안 잡혔다」가 되어 거짓**이 된다(M11).
 */
test("⚠올해 행에는 盗塁刺를 쓰지 않는다 — 0은 「없음」이 아니다(M11)", async () => {
  const { renderPlayerPage } = await import("../src/player-page.ts");
  const { playerPage } = await import("./fixtures.ts");
  const out = renderPlayerPage(playerPage(), context());
  assert.match(out, /盗塁刺は今シーズンの行にはありません/, "왜 없는지 말하지 않는다");
});

/**
 * ⚠**통산은 「작년까지 NPB + 올해 우리」다.**
 *
 * 처음에는 통산도 今季도 NPB 공표치에서 가져왔다. 한 줄 안의 뺄셈은 맞았지만
 * **NPB 페이지가 우리보다 며칠 늦어** 통산이 그만큼 낡았다 — 어제 친 홈런이 안 들어갔다.
 * (유저 지적: 「통산을 NPB 기준으로 맞추면 최신화가 늦는 거 아니야?」)
 *
 * 이어 붙일 수 있는 근거를 실측했다(2026-08-17):
 * · 끝난 시즌(2023~2025)에서는 두 출처가 **완전히 같다** — 타격 1,777건·투구 983건, 어긋남 0
 * · 진행 중 시즌에서는 **우리 쪽이 크거나 같다** — NPB 쪽이 큰 경우 **0명**(우리 누락 없음)
 * 그래서 경계에서 값이 튀지 않고, `통산 − 올해 = 작년까지` 도 그대로 성립한다.
 *
 * ⚠**화면이 그 구성과 근거를 말해야 한다** — 근거 없이 「이어 붙였다」고만 하면 값을 믿을 수 없다.
 */
test("⚠통산이 「작년까지 NPB + 올해 우리」라고, 그 근거와 함께 말한다", () => {
  const out = renderHomePage(data(), context());
  assert.match(out, /昨シーズンまではNPBの公表値、今季は当サイトの集計/, "구성을 말하지 않는다");
  assert.match(out, /数日遅れて/, "왜 그렇게 하는지 말하지 않는다");
  assert.match(out, /終わったシーズンでは両者が完全に一致/, "이어 붙여도 되는 근거가 없다");
  assert.match(out, /引き算も合います/, "뺄셈이 성립한다는 말이 없다");
});
