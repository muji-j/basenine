...
test("⚠구원 투수 페이지가 「勝率 순위를 안 낸다」는 사실과 이유를 말한다", () => {
  ...
  // ⚠**표창의 자격을 규정투구회라고 말하지 않는다**(2026-09-26 1차 대조) — 勝率第一位投手賞은 13勝以上이다.
  //   예전 문구 「最高勝率はNPBの規定投球回が資格」은 거짓이었고, 그 거짓이 이 각주의 근거였다.
  assert.ok(!b.includes("最高勝率はNPBの規定投球回が資格"), "표창의 자격을 규정투구회라고 적고 있다(거짓)");
  assert.match(b, /13勝以上/, "표창의 실제 자격(승수)이 없다");
});
