 ...
 *
 * ⚠**이것이 이 시험의 사각지대다**(2026-09-05 · 실물로 드러났다). 이 저장소는
 * **내부 주석을 한국어로 쓰므로 한글만 있는 조각은 통째로 검사 밖**이다 —
 * `layout.ts` 의 내비 주석에 **자라는 수(「2025 화면 2,307장」)가 오래 박혀 있었는데
 * 아무도 안 걸렸고**, 그 조각에 가타카나 라벨(`>ドラフト</a>`)이 붙는 순간 **처음으로 켜졌다.**
 * ⚠**그때 그 수는 이미 거짓이었다** — 경기 페이지를 날짜별로 묶어 2025 화면이 **1,088장**이 된 뒤였다.
 *
 * ⚠**지금 이 판정이 서 있는 것은 「우연히 옳은 불변식」 덕이다**: 제품 UI 가 **일본어 단일**이라
 * (`CLAUDE.md` §7) 「일본어 = 화면 · 한국어 = 주석」이 성립한다. **i18n 이 들어오는 날 깨진다.**
 *
 * ⚠**같은 구멍이 다른 조각에 몇 개인지는 「0건」이 아니라 「안 쟀음」이다.**
 */
