...
/**
 * ⚠**링크가 「그 날 파일 + 그 경기 앵커」로 바뀌었다**(2026-08-26 · 감사 P3 #41).
 * 경기 하나에 파일 하나였던 것을 **경기일 하나에 파일 하나**로 묶었다 —
 * Pages 파일 상한(20,000)에 여유가 2.61시즌뿐이었고 경기 페이지가 배포물의 48.6% 였다.
 * ⚠**앵커까지 본다.** 파일만 맞고 앵커가 틀리면 **그 날 첫 경기로 떨어진다** — 조용하고 발견이 늦다.
 */
test("경기 페이지가 있으면 링크를 낸다 — 그 날 파일의 그 경기 앵커로 간다", () => {
  const out = renderTodayPage(
    data({ games: [game({ gameId: "2026/0814/s-db-17" })] }),
    context(),
  );
  assert.match(out, /games\/2026-08-14\.html#g-2026-0814-s-db-17/);
});
