 ...
 * 아래 다섯은 **가시 텍스트 라벨**이 있고 그 라벨이 6.6 대비라
 * 1.4.11 위반으로 **단정할 수 없다**(감사자 판정 · 2026-09-08).
 * ⚠~~다섯은 `--hair-2` 테두리를 쓴다~~ 는 **2026-09-08 에 낡았다**(2차 검토 F4):
 * 2b 에서 `.tab` 과 `.hjump a` 의 쉬는 테두리를 **`transparent`** 로 껐다. 면제의 근거는
 * 테두리 잉크가 아니라 **글자가 컨트롤 안에 있다는 것**이므로 판정은 그대로 선다.
 * ⚠**`.hjump a` 는 그 뒤 목록에서 빠졌다**(2026-09-08 · 2f · 2차 검토 F8).
 * 2f 가 그 자리의 테두리를 **`--tx-3` 으로 되돌렸으므로**(`--page` 바탕에서 4.910 / 5.499)
 * **면제가 필요 없다** — 필요 없는 면제를 남겨 두면 다음 사람이 「그 자리는 원래 안 본다」로 읽는다.
 * ⚠**되돌린 이유는 따로다**: 「같은 묶음에 칠해진 형제가 있다」는 전제가 목차에서는
 * 대부분 성립하지 않아 **전부 맨 글자가 됐다**(사용자가 「가시성이 나쁘다」로 짚었다).
 * ⚠⚠**단, 그 판정은 「같은 묶음에 칠해진 형제가 있다」를 전제한다.**
 * 탭줄에서는 참이지만 **홀로 서는 토글**(`.mfind` 의 `規定到達のみ`)에는 그 형제가 없어
 * 테두리도 면도 없이 그려졌다(2차 검토 F1 · 빌드 실측 96개). 그래서 그 자리에만
 * `.mfind .tab:where(:not([aria-pressed="true"]))` 로 `--tx-3` 테두리를 따로 준다.
 * ⚠**다음에 홀로 서는 `.tab` 을 만들면 같은 함정에 빠진다 — 아래 시험이 그 자리를 지킨다.**
...
/**
 * ⚠**홀로 서는 토글은 탭줄의 면제를 물려받지 못한다**(2026-09-08 · 2차 검토 F1).
 *
 * `.tab` 의 면제 근거는 「글자가 컨트롤 안에 있다」이지만, 2b 가 쉬는 테두리를 껐기 때문에
 * **「같은 묶음에 칠해진 형제가 있다」가 같이 성립해야** 컨트롤의 넓이가 보인다.
 * `.mfind` 의 `規定到達のみ` 는 형제가 없어 그 전제가 깨졌고, 눌리지 않은 동안
 * **테두리도 면도 없이**(둘 다 `rgba(0,0,0,0)`) 그려졌다 — 빌드 실측 96개.
 *
 ...
 */
/**
 * ⚠**표식의 형태도 같아야 한다 — 잉크만 맞춰선 부족하다**(2026-09-08 · 2f · 2차 검토 F3).
 *
 * 2f 가 모서리 척도를 넣으면서 `.tnav a` 와 `.seasons a` 에 `border-radius:var(--r-pill)` 을 줬다.
 * 알약 반지름은 h&lt;w 인 상자에서 **h/2 로 클램프**되고 **inset 밑줄은 반지름에 잘린다** —
 * 남는 직선이 `w − h` 라 `.tnav a` 가 1280px 마우스에서 **40 → 13.4px**,
 * 390px 손가락에서 **약 5px** 가 됐다. `.brand` 만 반지름이 없어 **직선 그대로**였다.
 *
 * ⚠**바로 아래 시험은 `box-shadow` 값만 읽어서 그동안 초록이었다.**
 * 「같은 뜻을 같은 형태로」를 지키려면 **형태를 깎는 것**도 같이 봐야 한다.
 */
test("⚠밑줄로 말하는 표식에는 알약 반지름을 주지 않는다 — 반지름이 밑줄을 호로 깎는다", () => {
  const UNDERLINED = [".tnav a", ".brand", ".seasons a"];
  const rounded: string[] = [];
  for (const m of CSS_NC.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    const body = m[2]!;
    if (!/border-radius:\s*var\(--r-pill\)/.test(body)) continue;
    for (const one of m[1]!.split(",")) {
      const sel = one.trim();
      if (UNDERLINED.includes(sel)) rounded.push(sel);
    }
  }
  assert.deepEqual(
    rounded.sort(),
    [],
    "밑줄 표식을 쓰는 자리에 알약 반지름이 붙었다 — 표식이 호(弧)가 되고 셋의 형태가 갈라진다.\n" +
      "  장식이 필요하면 표식을 먼저 다른 형태로 바꿔라(.hjump a 는 테두리 색으로 말해서 알약과 안 싸운다).",
  );
});
...
test("⚠홀로 서는 토글(.mfind .tab)은 쉬는 동안에도 스스로 테두리를 갖는다", () => {
  ...
  assert.ok(
    m,
    ".mfind .tab 의 쉬는 테두리 규칙이 없다 — 눌리지 않은 토글이 테두리도 면도 없이 그려진다",
  );
  assert.match(
    m![1]!,
    /border-color:\s*var\(--tx-3\)/,
    "쉬는 테두리가 --tx-3 이 아니다 — 옆 .mfind input 과 같은 잉크여야 한 줄에서 어법이 맞는다",
  );
  // ⚠**눌린 것을 빼고 있는지도 못 박는다** — 안 빼면 채움색 위에 어긋난 고리가 생긴다(이 브랜치에서 세 번 났다)
  assert.ok(
    /\.mfind\s+\.tab:hover:where\(:not\(\[aria-pressed="true"\]\)\)/.test(CSS_NC),
    "호버 규칙이 눌린 것을 빼지 않는다 — 눌린 토글의 테두리가 채움색에서 벗어난다",
  );
  console.log("  · 홀로 서는 토글 .mfind .tab — 쉬는 테두리 --tx-3 · 호버 --tx-2 · 눌린 것은 제외");
});
