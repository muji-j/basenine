 ...
 * 실측: `<s>対 …</s>` **81,150곳 / 7,502장**이 전부 링크 아니었고, 고친 뒤 81,150/81,150 이 링크다.
 * ⚠**분모를 한 번 틀렸다** — 처음 적은 76,509 는 **이름 사전으로 본문을 훑는** 스캐너의 값이라
 *   1글자 이름과 「같은 문자종이 이어지는 자리」를 빼 과소치다(4,641 모자람).
 *   이 질문의 옷은 분모는 **자리 수**다 — `<s>対 ` 를 세라.
 *
