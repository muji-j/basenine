 ...
 *
 * ⚠**외부 요청 0회이고, 그것을 두 겹으로 지킨다.**
 *
 * ⑴ **초록일 때**: 아래 전건이 `PoliteFetcher` 를 만들기 **전에** `exit 2` 로 죽는다
 *    (진입점의 검사 순서: 연도 → 범위 → `--only` → 연락처 → `--delay` → **UA** → deps 생성).
 * ⑵ **붉을 때**: `--delay` 검사를 누가 되돌리면 흐름이 그 아래로 흘러간다. 그때를 위해
 *    **연락처를 일부러 `me@example.com`(RFC 2606 예약 도메인)으로 준다** —
 *    `buildUserAgent` 이 **deps 생성 전에** 그걸 거부하므로 **회귀해도 요청이 0이다.**
 *    ⚠**그 주소를 「실재하는 것」으로 바꾸지 마라. 여기서는 그게 걸쇠다.**
 *
 * ⚠**초판 머리말은 「외부 요청 0회」라고만 적었는데 그때는 사실이 아니었다** — 붉은 상태가
 * 실제로 npb.jp 를 쳤고(2026-09-05 · 222페이지), 본문 주석만 그 사실을 적어
 * **한 파일의 두 문장이 다른 말을 하고 있었다.** 이제 둘 다 참이다.
 *
...
test("⚠L1 하한 아래는 exit 2 — 경고만 하고 실사이트를 치지 않는다", () => {
  // ⚠⚠**이 본의 「빨간 상태」가 한때 실제로 네트워크를 쳤다.**
  //   하한을 넣기 전에 이 본을 `--from 2005`(기본값)로 썼다가 **222페이지를 0.583초 간격으로
  //   실제로 받아 버렸다**(우리 사이드카 `fetchedAt` 실측 · 128.9초). L1 위반을 막는 시험이
  //   L1 을 위반한 것이다. 지금은 **세 겹**으로 막혀 있고 셋 다 이유가 다르다:
  //   ⑴ `--from 2026 --to 2026` — 개최 전 연도라 회귀해도 **색인 1 + 연도 톱 1 = 2요청**이
  //      상한이다(`draftlist_*` 가 0건이라 3단계가 없다). ⚠**이 두 인자를 지우지 마라.**
  //   ⑵ `me@example.com` — `buildUserAgent` 이 예약 도메인을 **deps 생성 전에** 거부한다.
  //      ⚠**실재 주소로 바꾸면 이 걸쇠가 풀린다.**
  //   ⑶ 진짜 1차 방어는 `fetcher-archive.test.ts` 의 `parseDelayMs("500") === null` 이다 —
