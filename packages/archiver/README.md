...
8/14 경기 페이지에 8/15 경기 링크가 들어간다 — 헤더 내비가 다음 날 일정을 포함하도록
갱신되기 때문이다. 그래서 **하루 지나 같은 페이지를 다시 받으면 「신규」로 판정된다.**

| 무엇이 | 영향 |
|---|---|
| 파싱 결과 | **없다.** 표 내용은 그대로다 |
| 아카이브 블롭의 `revision` | **내비 변화만으로 오른다.** 「데이터가 정정됐다」는 뜻이 아니다 |
| DB의 `game.revision` | **의미 있다.** 상태가 실제로 바뀔 때만 오른다 |
| 일일 비용 | 최근 며칠분(하루 24장 정도)을 다시 저장한다. 무시할 수준 |

⚠**「어제 본 숫자와 다른데?」에 답할 때는 DB의 `revision`을 보라.** 블롭 쪽은 페이지 바이트가
바뀌었다는 사실만 말한다.
